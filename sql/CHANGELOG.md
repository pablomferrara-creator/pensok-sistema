...
| 2026-08-07 | `2026-08-07-vincular-productos-historico.sql` / `.down.sql` | Ambos | (ver commit "Vincular ventas/abastecimiento/devoluciones al producto real") | ✅ (corrido por Claude vía psql) | ✅ (corrido por Claude vía psql) | Agrega `producto_id` a `venta_items` (ya existía en Pilar) y `devolucion_items` (nuevo en ambos), + backfill por nombre exacto contra productos de hoy (excluye nombres duplicados). Resultados: Pilar 15.969/16.255 venta_items y 2.328/2.399 abastecimiento vinculados; Caamaño 385/396 y 424/431. **El down tiene una asimetría real entre proyectos — leer la advertencia dentro del archivo antes de correrlo, especialmente para `venta_items` en Pilar.** |
| 2026-08-10 | `2026-08-10-clientes-cuit.sql` / `.down.sql` | Ambos | (ver commit "Agregar CUIT a clientes") | ✅ (corrido por Claude vía psql) | ✅ (corrido por Claude vía psql) | Columna `clientes.cuit` (text, nullable). El down la borra junto con cualquier CUIT ya cargado — sin backfill posible, avisar antes de correrlo si ya hay datos. |
| 2026-08-11 | `2026-08-11-traspasos-editables.sql` / `.down.sql` | Ambos | (ver commit "Editar/eliminar traspasos") | ✅ (corrido por Claude vía psql) | ✅ (corrido por Claude vía psql) | Columnas `traspasos.pilar_id` (ambos, solo se usa en Caamaño) y `abastecimiento.traspaso_id` (ambos, FK a `traspasos(id)` on delete set null). **Además de correr el `.sql`, se hizo un backfill manual de `pilar_id`** en los 26 traspasos ya existentes de Caamaño, matcheando por (fecha,total) — los 26 matchearon únicos, sin ambigüedad, así que no quedó ninguno sin backfillear (script y SQL generado no se guardaron en el repo, quedaron en el scratchpad de la sesión). El down borra las dos columnas — el backfill de `pilar_id` no es reconstruible salvo repitiendo el mismo matcheo manual. |
