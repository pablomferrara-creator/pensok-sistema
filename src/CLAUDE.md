...
- **Frontend**: un solo archivo `App.jsx` (React/JSX, ~8.000 líneas), compilado con **esbuild**
...
```bash
npx esbuild App.jsx --loader:.jsx=jsx --jsx=automatic --bundle \
  --external:react --external:react-dom --external:@supabase/supabase-js \
  --external:jspdf --external:xlsx --format=esm --outfile=/dev/null
```
Si esto no tira error, el archivo compila. **Siempre correrlo antes de dar por terminado un cambio.**
