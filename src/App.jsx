...
function useData(toast){
  ...
  async function cargar(){
    ...
    const totalSaldo=(saldoData||[]).reduce((s,v)=>{
      ...
      if(!v.cobrado) return s+(v.total||0);
      return s;
    },0);
    const{data:trData}=await supabase.from("traspasos").select("saldo_pendiente").gt("saldo_pendiente",0);
    const totalTraspasos=(trData||[]).reduce((s,t)=>s+(t.saldo_pendiente||0),0);
    setTotalNosDeben(totalSaldo+totalTraspasos);
  ...
  async function registrarTraspaso(items, notas){
    ...
    for(const it of items){
      ...
      if(prod) await supabase.from("productos").update({stock:Math.max(0,(prod.stock||0)-it.cantidad)}).eq("id",it.id);
    }
...
function ModuloAnalisis({ventas,egresos,productos,vendedores,totalNosDeben,anioStats,onNavegar,onFiltroIngresos}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div className="psk-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>
...
// MODULO: INGRESOS
// ============================================================
function ModuloIngresos({ventas,vendedores,productos,clientes,onEditar,onEliminar,totalVentas,filtroInicial="",esAdmin=true}){
  const [fVend,setFV]=useState("Todos");
  const [fMet,setFM]=useState("Todos");
  const [fFecha,setFF]=useState(filtroInicial?"":hoy());
  const [fEstado,setFEstado]=useState(filtroInicial);
...
function ModuloEgresos({egresos,onRegistrar,onReembolsar,vendedores,proveedores,onEditar,onEliminar,esAdmin=true}){
  ...
  const [filtroReemb,setFiltroReemb]=useState(false);
  const filtrados=useMemo(()=>egresos.filter(e=>{if(filtroT!=="Todos"&&e.tipo!==filtroT)return false;if(filtroP!=="Todos"&&e.pagador!==filtroP)return false;if(filtroF&&e.fecha!==filtroF)return false;return true;}),[egresos,filtroReemb,filtroT,filtroP,filtroF]);
      ...
      {deudasPers.length>0&&(
        <Card style={{border:`1px solid #FFB80033`,background:"#FFB80006"}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
            ...
            {deudaPensok>0&&(
              <div style={{background:G.sup2,border:`1px solid #FF4D6A44`,borderRadius:10,padding:"10px 16px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                ...
                <div style={{fontSize:11,color:G.textoSec}}>Pensok debe a proveedores</div>
              </div>
            )}
          </div>
        </Card>
        ...
        <div className="psk-main" style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="ingresos"       && <ModuloIngresos       ventas={data.ventasConItems} vendedores={data.vendedores} productos={data.productos} clientes={data.clientes} onEditar={data.editarVenta} onEliminar={data.eliminarVenta} totalVentas={data.totalVentas} filtroInicial={filtroIngresos} esAdmin={esAdmin}/>}
