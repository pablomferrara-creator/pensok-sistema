...
function ModuloCaja({ventas,egresos,toast}){
  ...
  const vTransf    = ventasDia.filter(v=>v.metodo_pago==="Transferencia").reduce((s,v)=>s+(v.total||0),0);
  const vMP        = ventasDia.filter(v=>v.metodo_pago==="Mercado Pago").reduce((s,v)=>s+(v.total||0),0);
  const vDebito    = ventasDia.filter(v=>v.metodo_pago==="Debito").reduce((s,v)=>s+(v.total||0),0);
  const vCredito   = ventasDia.filter(v=>["Credito","Credito 1 pago","Credito cuotas"].includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0);
  ...
  const esperadoBanco = inicialBanco + vTransf               + movNet("banco");
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <Card>
              ...
              {[
                ...
                {l:"Transferencia",    v:vTransf,    c:G.azul},
                {l:"Mercado Pago",     v:vMP,        c:G.azul},
                {l:"Débito",           v:vDebito,    c:G.textoSec},
                {l:"Crédito",          v:vCredito,   c:G.textoSec},
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[
                    ...
                    {l:"Mercado Pago",          esp:esperadoMP,   real:realMP,   diff:diffMP,
                      det:`Inicial ${fmt(inicialMP)} + Ventas ${fmt(vMP)}${movNet("mp")!==0?` + Mov. ${fmt(movNet("mp"))}`:""}` },
                    {l:"Banco / Transferencia", esp:esperadoBanco,real:realBanco,diff:diffBanco,
                      det:`Inicial ${fmt(inicialBanco)} + Ventas ${fmt(vTransf)}${movNet("banco")!==0?` + Mov. ${fmt(movNet("banco"))}`:""}` },
                  ].map(x=>(
