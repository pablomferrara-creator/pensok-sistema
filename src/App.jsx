...
function useData(toast){
  ...
  async function eliminarVenta(id){
    // Reponer stock de cada item antes de eliminar
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  ...
                  <div style={{marginTop:8,fontSize:11,color:G.textoSec}}>
                    {(v.items||[]).map((it,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}>
                        <span>{it.nombre} <strong style={{color:G.texto}}>×{it.cantidad}</strong></span>
      ...
      {confirmarElim&&(
        <Modal title="Eliminar venta" onClose={()=>setConfirmarElim(null)}
          ...
          <p style={{color:G.textoSec,fontSize:13}}>¿Eliminar la venta de <strong style={{color:G.texto}}>{confirmarElim.cliente_nombre}</strong> del {confirmarElim.fecha} por <strong style={{color:G.texto}}>{fmt(confirmarElim.total)}</strong>? Esta acción no se puede deshacer.</p>
        </Modal>
      )}
    </div>
  );
