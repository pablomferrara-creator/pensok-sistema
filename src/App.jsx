...
function ModuloCaja({ventas,egresos,toast}){
  ...
  // Saldo inicial: del último cierre o lo que ingresa el usuario (primer cierre)
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {!esPrimerCierre&&cierreAnterior&&(
              <Card style={{background:G.sup2,border:`1px solid ${G.borde}`}}>
                <ST>Saldos base — cierre del {cierreAnterior.fecha}{diasSinCierre>0?` · ${diasSinCierre} día${diasSinCierre!==1?"s":""} acumulado${diasSinCierre!==1?"s":""}`:""}</ST>
...
export default function App(){
  ...
  // Sonido al recibir un pedido web nuevo
  const [ultimoCountPedidos,setUltimoCountPedidos] = useState(pedidosWebPend);
  useEffect(()=>{
    if(pedidosWebPend > ultimoCountPedidos && ultimoCountPedidos !== null){
      // Reproducir beep suave usando Web Audio API (no requiere archivo)
      try{
        ...
        if(AC){
          ...
          osc.frequency.value = 880; // La5, suave
          osc.type = "sine";
          ...
          // Segundo tono más alto para hacer un "ding-ding"
          setTimeout(()=>{
            try{
              const ctx2 = new AC();
      ...
      toast.ok(`🛎 Nuevo pedido web recibido (${pedidosWebPend - ultimoCountPedidos} ${pedidosWebPend - ultimoCountPedidos===1?"nuevo":"nuevos"})`);
    }
    setUltimoCountPedidos(pedidosWebPend);
  },[pedidosWebPend]);
