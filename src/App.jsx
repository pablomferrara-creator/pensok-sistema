...
function ModuloEgresos({egresos,onRegistrar,onReembolsar,vendedores,proveedores,onEditar,onEliminar,esAdmin=true}){
  ...
  async function guardar(){
    ...
    await onRegistrar({
      ...
      pagador:fPagador,reembolso_pendiente:reembolso||deudaProv,reembolsado:false,
      monto_reembolsado:montoAbonado<montoT?montoAbonado:0,
...
function ModuloCaja({ventas,egresos,toast}){
  ...
  // ── Helper: calcular movimientos netos de un bolsillo en un rango de fechas ──
  const movNetRango = (bolsillo, desde, hasta) => {
    const movs = movimientos.filter(m=>m.fecha>=desde&&m.fecha<=hasta);
    const entra = movs.filter(m=>m.destino===bolsillo).reduce((s,m)=>s+(m.monto||0),0);
    const sale  = movs.filter(m=>m.origen===bolsillo).reduce((s,m)=>s+(m.monto||0),0);
    return entra - sale;
  };

  // ── Helper: sumar ventas/egresos de un rango ──
  const ventasRango = (desde, hasta) => ventas.filter(v=>v.fecha>=desde&&v.fecha<=hasta);
  const egresosRango = (desde, hasta) => egresos.filter(e=>e.fecha>=desde&&e.fecha<=hasta);
  const pagosRango  = (desde, hasta) => pagosDia.filter(p=>p.fecha>=desde&&p.fecha<=hasta);

  const calcMovsBolsillo = (vts, egs, pgs, bolsillo) => {
    const cobros = pgs.filter(p=>p.tipo==="ingreso");
    const reembs = pgs.filter(p=>p.tipo==="egreso");
    if(bolsillo==="caja_chica"){
      ...
      const vEf   = vts.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+(v.total||0),0)+cobEf;
      const gEf   = egs.filter(e=>e.metodo_pago==="Efectivo").reduce((s,e)=>s+(e.monto||0),0)+reEf;
      return vEf - gEf;
    }
    if(bolsillo==="mp"){
      const cobMP = cobros.filter(p=>MP_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
      const vMP_  = vts.filter(v=>MP_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0)+cobMP;
      return vMP_;
    }
    if(bolsillo==="banco"){
      const cobBa = cobros.filter(p=>BANCO_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
      const vBa   = vts.filter(v=>BANCO_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0)+cobBa;
      return vBa;
    }
    return 0;
  };

  // ── Datos SOLO del día seleccionado ──
  const ventasDia     = ventas.filter(v=>v.fecha===fecha);
  const egresosDia    = egresos.filter(e=>e.fecha===fecha);
  const pagosDiaFecha = pagosDia.filter(p=>p.fecha===fecha);
  const movsDia       = movimientos.filter(m=>m.fecha===fecha);
  const movNet = (bolsillo) => movNetRango(bolsillo, fecha, fecha);

  const vEfectivo = calcMovsBolsillo(ventasDia, egresosDia, pagosDiaFecha, "caja_chica");
  const vMP       = calcMovsBolsillo(ventasDia, egresosDia, pagosDiaFecha, "mp");
  const vBanco    = calcMovsBolsillo(ventasDia, egresosDia, pagosDiaFecha, "banco");
  ...
  const gEfectivo = egresosDia.filter(e=>e.metodo_pago==="Efectivo").reduce((s,e)=>s+(e.monto||0),0)
    + pagosDiaFecha.filter(p=>p.tipo==="egreso"&&p.metodo_pago==="Efectivo").reduce((s,p)=>s+p.monto,0);
  ...
  // ── Cálculo ACUMULADO: desde el día posterior al último cierre hasta hoy ──
  // Si hay cierre anterior, acumular desde el día siguiente a ese cierre hasta la fecha seleccionada.
  // Si no hay cierre anterior, acumular todo.
  const fechaDesdeAcum = cierreAnterior
    ? (()=>{ const d=new Date(cierreAnterior.fecha+"T00:00:00"); d.setDate(d.getDate()+1); return d.toISOString().slice(0,10); })()
    : "2000-01-01";

  const diasSinCierre = cierreAnterior
    ? (()=>{
        const d1=new Date(fechaDesdeAcum+"T00:00:00");
        const d2=new Date(fecha+"T00:00:00");
        return Math.round((d2-d1)/(1000*60*60*24));
      })()
    : 0;

  const ventasAcum  = ventasRango(fechaDesdeAcum, fecha);
  const egresosAcum = egresosRango(fechaDesdeAcum, fecha);
  const pagosAcum   = pagosRango(fechaDesdeAcum, fecha);

  const acumCaja  = calcMovsBolsillo(ventasAcum, egresosAcum, pagosAcum, "caja_chica") + movNetRango("caja_chica", fechaDesdeAcum, fecha);
  const acumMP    = calcMovsBolsillo(ventasAcum, egresosAcum, pagosAcum, "mp")         + movNetRango("mp",         fechaDesdeAcum, fecha);
  const acumBanco = calcMovsBolsillo(ventasAcum, egresosAcum, pagosAcum, "banco")      + movNetRango("banco",      fechaDesdeAcum, fecha);

  // Esperado acumulado = saldo del último cierre + todo lo acumulado desde entonces
  const esperadoAcumCaja  = inicialCaja  + acumCaja;
  const esperadoAcumMP    = inicialMP    + acumMP;
  const esperadoAcumBanco = inicialBanco + acumBanco;

  // Esperado solo del día (para mostrar diferencia del día por separado)
  const esperadoCaja  = esperadoAcumCaja;
  const esperadoMP    = esperadoAcumMP;
  const esperadoBanco = esperadoAcumBanco;
  ...
  // Diferencia acumulada (lo que importa)
  const diffCaja   = realCaja   - esperadoAcumCaja;
  const diffMP     = realMP     - esperadoAcumMP;
  const diffBanco  = realBanco  - esperadoAcumBanco;

  // Diferencia solo del día (para información adicional)
  const espSoloDiaCaja  = (cierreAnterior?.saldo_caja_chica||inicialCaja) + calcMovsBolsillo(ventasDia,egresosDia,pagosDiaFecha,"caja_chica") + movNet("caja_chica");
  const espSoloDiaMP    = (cierreAnterior?.saldo_mp||inicialMP)           + calcMovsBolsillo(ventasDia,egresosDia,pagosDiaFecha,"mp")         + movNet("mp");
  const espSoloDiaBanco = (cierreAnterior?.saldo_banco||inicialBanco)     + calcMovsBolsillo(ventasDia,egresosDia,pagosDiaFecha,"banco")      + movNet("banco");
  const diffSoloDiaCaja  = realCaja  - espSoloDiaCaja;
  const diffSoloDiaMP    = realMP    - espSoloDiaMP;
  const diffSoloDiaBanco = realBanco - espSoloDiaBanco;
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  ...
                  {diasSinCierre>0&&(
                    <span style={{fontSize:11,color:G.amarillo,background:"#FFB80015",border:"1px solid #FFB80033",borderRadius:6,padding:"2px 8px"}}>
                      ⚠ {diasSinCierre} día{diasSinCierre>1?"s":""} sin cierre acumulado{diasSinCierre>1?"s":""}
                    </span>
                  )}
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[
                    {l:"Caja Chica",  esp:esperadoAcumCaja,  real:realCaja,  diffAcum:diffCaja,  diffDia:diffSoloDiaCaja,
                      det:`Desde cierre ${cierreAnterior?.fecha||"inicial"} · Saldo base ${fmt(inicialCaja)} + acum. efectivo`},
                    {l:"Mercado Pago",esp:esperadoAcumMP,    real:realMP,    diffAcum:diffMP,    diffDia:diffSoloDiaMP,
                      det:`Desde cierre ${cierreAnterior?.fecha||"inicial"} · Saldo base ${fmt(inicialMP)} + acum. MP`},
                    {l:"Banco",       esp:esperadoAcumBanco, real:realBanco, diffAcum:diffBanco, diffDia:diffSoloDiaBanco,
                      det:`Desde cierre ${cierreAnterior?.fecha||"inicial"} · Saldo base ${fmt(inicialBanco)} + acum. banco`},
                  ].map(x=>(
                    <div key={x.l} style={{background:G.sup2,borderRadius:8,padding:"10px 14px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                        ...
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          {diasSinCierre>0&&(
                            <span style={{fontSize:11,color:diffColor(x.diffDia),background:G.sup,border:`1px solid ${G.borde}`,borderRadius:5,padding:"1px 7px"}}>
                              Hoy: {diffLabel(x.diffDia)}
                            </span>
                          )}
                          <span style={{fontSize:13,fontWeight:700,color:diffColor(x.diffAcum)}}>
                            Acum: {diffLabel(x.diffAcum)}
                          </span>
                        </div>
                      </div>
                      ...
                      <div style={{display:"flex",gap:16,fontSize:11,color:G.textoSec}}>
                        <span>Esperado acum.: <strong style={{color:G.texto}}>{fmt(x.esp)}</strong></span>
