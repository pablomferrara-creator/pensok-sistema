...
function ModuloIngresos({ventas,vendedores,productos,clientes,onEditar,onEliminar,totalVentas,filtroInicial="",esAdmin=true}){
  ...
  async function guardarVenta(){
    ...
    const comision=parseFloat(evComision)||0;
    await onEditar(editandoV.id,{cliente_nombre:evCliente,vendedor:evVendedor,metodo_pago:evMetodo,cobrado:evCobrado,entregado:evEntregado,comision_plataforma:comision,total,ganancia:total-costos-comision});
    await supabase.from("venta_items").delete().eq("venta_id",editandoV.id);
