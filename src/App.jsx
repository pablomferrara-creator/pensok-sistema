...
function ModuloIngresos({ventas,vendedores,onEditar,onEliminar,totalVentas}){
  ...
  async function guardarVenta(){
    ...
    // Recalcular total y ganancia desde los items editados
    const itemsNum = evItems.map(i=>({
      ...i,
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {editandoV&&(()=>{
        ...
        const bruto=itemsNum.reduce((s,i)=>s+i.precio*i.cantidad,0);
        const desc=editandoV.descuento||0;
        const totalPreview=Math.round(bruto*(1-desc/100));
        const ganBruta=itemsNum.reduce((s,i)=>s+(i.precio-(i.costo||0))*i.cantidad,0);
        const comision=parseFloat(evComision)||0;
        ...
        <Modal title="Editar venta" onClose={()=>setEditandoV(null)} maxWidth={640}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {/* Comision — solo si pago electronico */}
            {conComision&&(
              <div style={{background:"#4D9EFF11",border:"1px solid #4D9EFF33",borderRadius:8,padding:"10px 14px"}}>
                ...
                {comision>0&&<div style={{fontSize:11,color:G.textoSec,marginTop:6}}>Ganancia neta luego de comision: <strong style={{color:G.verde}}>{fmt(ganPreview)}</strong></div>}
              </div>
            )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              {evItems.map((item,idx)=>(
                <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 32px",gap:8,alignItems:"center"}}>
                  <input value={item.nombre} onChange={e=>actualizarItem(idx,"nombre",e.target.value)}
                    style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:7,padding:"7px 10px",color:G.texto,fontSize:12,outline:"none",width:"100%"}}/>
              ...
              <Btn small variant="secondary" onClick={agregarItem} style={{alignSelf:"flex-start",marginTop:4}}>+ Agregar producto</Btn>
            </div>

            {/* Resumen ganancia */}
            {desc>0&&<div style={{fontSize:12,color:G.textoSec}}>Descuento aplicado: <strong style={{color:G.amarillo}}>{desc}%</strong></div>}
            ...
            <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between",fontSize:13}}>
              <span style={{color:G.textoSec}}>Ganancia neta{comision>0?` (−${fmt(comision)} comision)`:""}</span>
