...
function useData(toast){
  ...
  async function cargar(){
    ...
    setAnioStats({facturacion:(anioData||[]).reduce((s,v)=>s+(v.total||0),0),ganancia:(anioData||[]).reduce((s,v)=>s+(v.ganancia||0),0),cantidad:(anioData||[]).length});
    setLoading(false);
  }
...
function ModuloAnalisis({ventas,egresos,productos,vendedores,totalNosDeben,anioStats,onNavegar,onFiltroIngresos}){
  ...
  // totalNosDeben viene directo de Supabase via useData (no limitado a 5000)
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
        ...
        <MetricCard label="Nos deben" value={fmt(totalNosDeben)} color={G.azul} sub={`${sinCobrar.length} ventas pendientes`} accent={"#4D9EFF44"}/>
      </div>
...
function ModuloVenta({clientes,productos,onRegistrar,vendedores}){
  ...
  const DESC_POR_METODO = {"Efectivo":10,"Transferencia MP":5,"Transferencia Banco":5,"Debito MP":0,"Debito Banco":0,"Credito MP":0,"Credito Banco":0,"Credito Cuotas Banco":0};
  ...
  function cambiarMetodo(m){setMetodo(m);setDescuento(String({"Efectivo":10,"Transferencia MP":5,"Transferencia Banco":5}.hasOwnProperty(m)?{"Efectivo":10,"Transferencia MP":5,"Transferencia Banco":5}[m]:0));}
  const [clienteId, setClienteId] = useState("");
...
function ModuloIngresos({ventas,vendedores,productos,onEditar,onEliminar,totalVentas,filtroInicial=""}){
  ...
  function agregarItem(){
    setEvItems(prev=>[...prev,{nombre:"",cantidad:"1",precio:"0",costo:0}]);
  }
              ...
              <div>
                ...
                <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                  ...
                  {v.metodo_pago!=="Efectivo"&&!(v.comision_plataforma>0)&&(
                    <span style={{background:"#FFB80022",color:G.amarillo,border:"1px solid #FFB80055",borderRadius:6,padding:"2px 9px",fontSize:11,fontWeight:600,letterSpacing:0.3}} title="Falta cargar la comisión de plataforma">⚠ Sin comisión</span>
                  )}
                  {v.metodo_pago!=="Efectivo"&&v.comision_plataforma>0&&(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              {evItems.map((item,idx)=>(
                <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 32px",gap:8,alignItems:"center"}}>
                  ...
                  <button onClick={()=>eliminarItem(idx)} style={{background:"#FF4D6A18",border:"1px solid #FF4D6A33",borderRadius:7,color:G.rojo,cursor:"pointer",fontSize:14,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center"}}>x</button>
                </div>
              ))}
      ...
      <div style={{minHeight:"100vh",background:G.fondo}}>
        ...
        <div style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              {modulo==="analisis"       && <ModuloAnalisis       ventas={data.ventasConItems} egresos={data.egresos} productos={data.productos} vendedores={data.vendedores} totalNosDeben={data.totalNosDeben} anioStats={data.anioStats||{facturacion:0,ganancia:0,cantidad:0}} onNavegar={setModulo} onFiltroIngresos={setFiltroIngresos}/>}
...
function ModuloCaja({ventas,egresos,toast}){
  ...
  const [cajChica,setCajChica]=useState("");const [saldoMP,setSaldoMP]=useState("");
  const [saldoBanco,setSaldoBanco]=useState("");const [saldoAhorro,setSaldoAhorro]=useState("");
  const [notas,setNotas]=useState("");const [loading,setLoading]=useState(false);
  const [cierres,setCierres]=useState([]);const [movimientos,setMovimientos]=useState([]);
  const [tab,setTab]=useState("nuevo");
  const [modalMov,setModalMov]=useState(false);
  const [movOrigen,setMovOrigen]=useState("caja_chica");const [movDestino,setMovDestino]=useState("banco");
  const [movMonto,setMovMonto]=useState("");const [movConcepto,setMovConcepto]=useState("");
  const [loadingMov,setLoadingMov]=useState(false);
  const [initCaja,setInitCaja]=useState("");const [initMP,setInitMP]=useState("");
  const [initBanco,setInitBanco]=useState("");const [initAhorro,setInitAhorro]=useState("");

  const BOLSILLOS=[{key:"caja_chica",label:"Caja Chica"},{key:"mp",label:"Mercado Pago"},{key:"banco",label:"Banco"},{key:"ahorro",label:"Ahorro"}];
  ...
  async function cargarHistorial(){
    ...
    setCierres(c||[]);setMovimientos(m||[]);
  }

  const cierreAnterior=useMemo(()=>cierres.find(c=>c.fecha<fecha)||null,[cierres,fecha]);
  ...
  const MP_MET=["Transferencia MP","Debito MP","Credito MP"];
  const BANCO_MET=["Transferencia Banco","Debito Banco","Credito Banco","Credito Cuotas Banco"];
  const vEfectivo=ventasDia.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+(v.total||0),0);
  const vMP=ventasDia.filter(v=>MP_MET.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0);
  const vBanco=ventasDia.filter(v=>BANCO_MET.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0);
  ...
  const movNet=b=>{const e=movsDia.filter(m=>m.destino===b).reduce((s,m)=>s+(m.monto||0),0);const o=movsDia.filter(m=>m.origen===b).reduce((s,m)=>s+(m.monto||0),0);return e-o;};
  ...
  const realCaja=parseFloat(cajChica)||0;const realMP=parseFloat(saldoMP)||0;
  const realBanco=parseFloat(saldoBanco)||0;const realAhorro=parseFloat(saldoAhorro)||0;
  const diffCaja=realCaja-esperadoCaja;const diffMP=realMP-esperadoMP;const diffBanco=realBanco-esperadoBanco;

  async function guardarCierre(){
    if(!fecha){toast.err("Seleccioná una fecha");return;}setLoading(true);
    const{error}=await supabase.from("cierres_caja").upsert({
      fecha,saldo_caja_chica:realCaja,saldo_mp:realMP,saldo_banco:realBanco,saldo_ahorro:realAhorro,
      ventas_efectivo:vEfectivo,ventas_mp:vMP,ventas_transferencia:vBanco,ventas_total:vTotal,
      gastos_efectivo:gEfectivo,gastos_otros:gOtros,gastos_total:gTotal,ganancia_neta:ganNeta,
      diff_caja_chica:diffCaja,diff_mp:diffMP,diff_banco:diffBanco,notas,
    },{onConflict:"fecha"});
    if(error){toast.err("Error al guardar: "+error.message);setLoading(false);return;}
    toast.ok("Cierre guardado");setLoading(false);cargarHistorial();setTab("historial");
  }
  async function guardarMovimiento(){
    if(!movMonto||movOrigen===movDestino){toast.err("Revisá los datos");return;}setLoadingMov(true);
    await supabase.from("movimientos_caja").insert({fecha,origen:movOrigen,destino:movDestino,monto:parseFloat(movMonto)||0,concepto:movConcepto});
    toast.ok("Movimiento registrado");setLoadingMov(false);setModalMov(false);setMovMonto("");setMovConcepto("");cargarHistorial();
  }
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {[{k:"nuevo",l:"Nuevo cierre"},{k:"historial",l:"Historial"}].map(t=>(
          <button key={t.k} onClick={()=>setTab(t.k)} style={{background:tab===t.k?G.verde:G.sup2,color:tab===t.k?"#000":G.textoSec,border:`1px solid ${tab===t.k?G.verde:G.borde}`,borderRadius:8,padding:"6px 18px",fontSize:12,fontWeight:600,cursor:"pointer"}}>{t.l}</button>
        ))}
        <div style={{flex:1}}/>
        <Btn small variant="secondary" onClick={()=>setModalMov(true)}>↔ Registrar movimiento</Btn>
      </div>
      {tab==="nuevo"&&(<>
        <Card><div style={{display:"flex",gap:12,alignItems:"flex-end",flexWrap:"wrap"}}>
          <Fi label="Fecha del cierre" value={fecha} onChange={setFecha} type="date" style={{width:200}}/>
          <div style={{fontSize:13,color:G.textoSec}}>{ventasDia.length} ventas · {egresosDia.length} egresos</div>
        </div></Card>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <Card><ST>Ingresos del día — sistema</ST>
              {[{l:"Efectivo",v:vEfectivo,c:G.verde},{l:"Mercado Pago",v:vMP,c:G.azul},{l:"Banco",v:vBanco,c:G.azul}].filter(x=>x.v>0).map(x=>(
              ...
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",fontSize:14,fontWeight:600}}>
                <span>Total ventas</span><span style={{fontFamily:"DM Mono,monospace",color:G.verde}}>{fmt(vTotal)}</span>
              </div>
            </Card>
            <Card><ST>Egresos del día — sistema</ST>
              {[{l:"Efectivo",v:gEfectivo,c:G.rojo},{l:"Otros",v:gOtros,c:G.naranja}].filter(x=>x.v>0).map(x=>(
                <div key={x.l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:13}}>
                  <span style={{color:G.textoSec}}>{x.l}</span><span style={{fontFamily:"DM Mono,monospace",color:x.c}}>{fmt(x.v)}</span>
                </div>
              ))}
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",fontSize:14,fontWeight:600}}>
                <span>Total gastos</span><span style={{fontFamily:"DM Mono,monospace",color:G.rojo}}>{fmt(gTotal)}</span>
              </div>
            </Card>
            <Card style={{border:`1px solid ${G.verde}33`}}><ST>Ganancia neta del día</ST>
              <div style={{fontSize:28,fontWeight:700,color:G.verde,fontFamily:"DM Mono,monospace"}}>{fmt(ganNeta)}</div>
            </Card>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {esPrimerCierre&&(
              <Card style={{border:`1px solid ${G.amarillo}44`}}><ST>⚡ Primer cierre — saldos antes de hoy</ST>
                <div style={{fontSize:12,color:G.textoSec,marginBottom:10}}>Ingresá cuánto había en cada bolsillo ANTES de las operaciones de hoy.</div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            {!esPrimerCierre&&cierreAnterior&&(
              <Card style={{background:G.sup2}}><ST>Saldos iniciales — cierre del {cierreAnterior.fecha}</ST>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:12}}>
                  {[{l:"Caja",v:inicialCaja},{l:"MP",v:inicialMP},{l:"Banco",v:inicialBanco}].map(x=>(
                    <div key={x.l} style={{display:"flex",justifyContent:"space-between"}}><span style={{color:G.textoSec}}>{x.l}</span><span style={{fontFamily:"DM Mono,monospace",fontWeight:600}}>{fmt(x.v)}</span></div>
                  ))}
                </div>
              </Card>
            )}
            <Card><ST>Saldos reales al cierre</ST>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                <Fi label="Caja Chica" value={cajChica} onChange={setCajChica} type="number" placeholder="0"/>
                ...
                <Fi label="Ahorro (informativo)" value={saldoAhorro} onChange={setSaldoAhorro} type="number" placeholder="0"/>
              </div>
            </Card>
            {(cajChica||saldoMP||saldoBanco)&&(<Card><ST>Diferencias</ST>
              {[
                {l:"Caja Chica",esp:esperadoCaja,real:realCaja,diff:diffCaja,det:`Inicial ${fmt(inicialCaja)} + Ef. ${fmt(vEfectivo)} − Gas. ${fmt(gEfectivo)}`},
                {l:"Mercado Pago",esp:esperadoMP,real:realMP,diff:diffMP,det:`Inicial ${fmt(inicialMP)} + MP ${fmt(vMP)}`},
                {l:"Banco",esp:esperadoBanco,real:realBanco,diff:diffBanco,det:`Inicial ${fmt(inicialBanco)} + Banco ${fmt(vBanco)}`},
              ].map(x=>(
                <div key={x.l} style={{background:G.sup2,borderRadius:8,padding:"10px 14px",marginBottom:8}}>
                  ...
                  <div style={{fontSize:10,color:G.textoSec,marginBottom:3}}>{x.det}</div>
            ...
            <Card><Fi label="Notas del cierre" value={notas} onChange={setNotas} rows={2} placeholder="Observaciones..."/></Card>
            <Btn full onClick={guardarCierre} disabled={loading}>{loading?<span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><Spinner/>Guardando...</span>:"💾 Guardar cierre de caja"}</Btn>
          </div>
        </div>
      </>)}
          ...
          <Card key={c.id}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,textAlign:"center"}}>
                {[{l:"Caja",v:c.saldo_caja_chica,d:c.diff_caja_chica},{l:"MP",v:c.saldo_mp,d:c.diff_mp},{l:"Banco",v:c.saldo_banco,d:c.diff_banco},{l:"Ahorro",v:c.saldo_ahorro,d:null}].map(x=>(
            ...
            {c.notas&&<div style={{marginTop:8,fontSize:12,color:G.textoSec,fontStyle:"italic"}}>"{c.notas}"</div>}
            {movimientos.filter(m=>m.fecha===c.fecha).length>0&&(
              <div style={{marginTop:8,borderTop:`1px solid ${G.borde}22`,paddingTop:6}}>
                <div style={{fontSize:11,color:G.textoSec,fontWeight:600,marginBottom:4}}>MOVIMIENTOS</div>
                {movimientos.filter(m=>m.fecha===c.fecha).map(m=>(
                  <div key={m.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"2px 0",color:G.textoSec}}>
                    <span>{BOLSILLOS.find(b=>b.key===m.origen)?.label} → {BOLSILLOS.find(b=>b.key===m.destino)?.label}{m.concepto?` · ${m.concepto}`:""}</span>
                    <span style={{color:G.azul,fontFamily:"DM Mono,monospace"}}>{fmt(m.monto)}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {modalMov&&(
        <Modal title="Registrar movimiento" onClose={()=>setModalMov(false)}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.textoSec}}>Fecha: {fecha}</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            {movOrigen===movDestino&&<div style={{fontSize:12,color:G.rojo}}>Origen y destino no pueden ser iguales</div>}
