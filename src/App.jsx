                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[
                    {l:"Caja Chica",  esp:esperadoCaja,   real:realCaja,   diff:diffCaja,
                      det:`Arranque ${fmt(arranqueCaja)} + ventas ef. ${fmt(vEfectivo)}${cobDeudaEf>0?` + cobros deuda ef. ${fmt(cobDeudaEf)}`:""}${gEfectivo>0?` − gastos ef. ${fmt(gEfectivo)}`:""}${reembDeudaEf>0?` − reembolsos ef. ${fmt(reembDeudaEf)}`:""}${movNetoCaja!==0?` ${movNetoCaja>0?"+":"−"} movimientos ${fmt(Math.abs(movNetoCaja))}`:""}`},
                    {l:"Mercado Pago",esp:esperadoMP,     real:realMP,     diff:diffMP,
                      det:`Arranque ${fmt(arranqueMP)} + ventas MP ${fmt(vMP)}${cobDeudaMP>0?` + cobros deuda MP ${fmt(cobDeudaMP)}`:""}${gMPtot>0?` − gastos MP ${fmt(gMPtot)}`:""}${reembDeudaMP>0?` − reembolsos MP ${fmt(reembDeudaMP)}`:""}${movNetoMP!==0?` ${movNetoMP>0?"+":"−"} movimientos ${fmt(Math.abs(movNetoMP))}`:""}`},
                    {l:"Banco",       esp:esperadoBanco,  real:realBanco,  diff:diffBanco,
                      det:`Arranque ${fmt(arranqueBanco)} + ventas banco ${fmt(vBanco)}${cobDeudaBanco>0?` + cobros deuda banco ${fmt(cobDeudaBanco)}`:""}${gBancoTot>0?` − gastos banco ${fmt(gBancoTot)}`:""}${reembDeudaBanco>0?` − reembolsos banco ${fmt(reembDeudaBanco)}`:""}${movNetoBanco!==0?` ${movNetoBanco>0?"+":"−"} movimientos ${fmt(Math.abs(movNetoBanco))}`:""}`},
