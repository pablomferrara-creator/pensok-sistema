...
function useData(toast){
  ...
  return{clientes,productos,ventasConItems,egresos,abastecimiento,vendedores,proveedores,tipoCambio,totalVentas,totalNosDeben,loading,cargar,registrarVenta,registrarEgreso,marcarReembolsado,guardarCliente,guardarProducto,registrarAbastecimiento,guardarVendedor,toggleVendedor,guardarProveedor,toggleProveedor,editarVenta,eliminarVenta,editarEgreso,eliminarEgreso,editarAbastecimiento,eliminarAbastecimiento,actualizarTipoCambio,actualizarPorcentaje,actualizarDesdeCSV};
}

// ============================================================
// MODULO: ANALISIS / DASHBOARD
// ============================================================
function ModuloAnalisis({ventas,egresos,productos,vendedores,totalNosDeben}){
  const hoyStr=hoy();const mes=mesAct();const anio=new Date().getFullYear().toString();
  const [periodo,setPeriodo]=useState("mes");

  // ── Filtros por periodo ──
  const vHoy=ventas.filter(v=>v.fecha===hoyStr);
  const vMes=ventas.filter(v=>v.fecha?.startsWith(mes));
  const vAnio=ventas.filter(v=>v.fecha?.startsWith(anio));
  const eHoy=egresos.filter(e=>e.fecha===hoyStr);
  const eMes=egresos.filter(e=>e.fecha?.startsWith(mes));
  const eAnio=egresos.filter(e=>e.fecha?.startsWith(anio));

  // ── Seleccion activa ──
  const vSel=periodo==="hoy"?vHoy:periodo==="mes"?vMes:vAnio;
  const eSel=periodo==="hoy"?eHoy:periodo==="mes"?eMes:eAnio;

  // ── Metricas ventas ──
  const facturacion=vSel.reduce((s,v)=>s+(v.total||0),0);
  const gananciaNeta=vSel.reduce((s,v)=>s+(v.ganancia||0),0);
  const pctGanancia=facturacion>0?Math.round(gananciaNeta/facturacion*100):0;
  const ticketProm=vSel.length>0?Math.round(facturacion/vSel.length):0;
  ...
  const porMet=METODOS_PAGO.map(m=>({m,total:vMes.filter(v=>v.metodo_pago===m).reduce((s,v)=>s+(v.total||0),0),cant:vMes.filter(v=>v.metodo_pago===m).length})).filter(x=>x.cant>0).sort((a,b)=>b.total-a.total);
  ...
  const periodos=[{k:"hoy",l:"Hoy"},{k:"mes",l:"Este mes"},{k:"anio",l:"Este año"}];
  const labelPeriodo=periodos.find(p=>p.k===periodo)?.l||"";
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        {periodos.map(p=>(
          <button key={p.k} onClick={()=>setPeriodo(p.k)}
            style={{background:periodo===p.k?G.verde:G.sup2,color:periodo===p.k?"#000":G.textoSec,border:`1px solid ${periodo===p.k?G.verde:G.borde}`,borderRadius:8,padding:"6px 16px",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all .15s"}}>
            {p.l}
          </button>
        ))}
        <span style={{fontSize:12,color:G.textoSec,marginLeft:4}}>{vSel.length} ventas</span>
      </div>

      {/* Fila 1: Ventas y Ganancia */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        <MetricCard label={`Facturación Neta — ${labelPeriodo}`} value={fmt(facturacion)} sub={`${vSel.length} ventas`} color={G.verde} accent={"#00C48C33"}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <MetricCard label="Ganancia Real" value={fmt(gananciaReal)} color={gananciaReal>=0?G.verde:G.rojo} sub={`Gan. Neta − Gastos (${fmt(totalGastos)})`} accent={gananciaReal>=0?"#00C48C22":"#FF4D6A22"}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>
        ...
        <MetricCard label="Sin entregar" value={fmtNum(sinEntregar.length)} color={G.amarillo} sub="ventas pendientes de entrega" accent={"#FFB80033"}/>
      </div>
...
function ModuloVenta({clientes,productos,onRegistrar,vendedores}){
  const METODOS_VENTA = ["Efectivo","Transferencia","Debito","Credito 1 pago","Credito cuotas"];
  const DESC_POR_METODO = {"Efectivo":10,"Transferencia":5,"Debito":0,"Credito 1 pago":0,"Credito cuotas":0};
...
// MODULO: INGRESOS
// ============================================================
function ModuloIngresos({ventas,vendedores,productos,onEditar,onEliminar,totalVentas}){
  ...
  const METODOS_CON_COMISION = ["Transferencia","Debito","Credito","Mercado Pago"];
      ...
      <div style={{minHeight:"100vh",background:G.fondo}}>
        ...
        <div style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              {modulo==="analisis"       && <ModuloAnalisis       ventas={data.ventasConItems} egresos={data.egresos} productos={data.productos} vendedores={data.vendedores} totalNosDeben={data.totalNosDeben}/>}
              ...
              {modulo==="ingresos"       && <ModuloIngresos       ventas={data.ventasConItems} vendedores={data.vendedores} productos={data.productos} onEditar={data.editarVenta} onEliminar={data.eliminarVenta} totalVentas={data.totalVentas}/>}
