...
function useData(toast){
  ...
  return{clientes,productos,ventasConItems,egresos,abastecimiento,vendedores,proveedores,loading,cargar,registrarVenta,registrarEgreso,marcarReembolsado,guardarCliente,guardarProducto,registrarAbastecimiento,guardarVendedor,toggleVendedor,guardarProveedor,toggleProveedor};
}
...
// MODULO: EGRESOS
// ============================================================
function ModuloEgresos({egresos,onRegistrar,onReembolsar,vendedores,proveedores}){
...
function ModuloProductos({productos,onGuardar,proveedores}){
  ...
  function abrirEditar(p){
    ...
    // Calcular margenes desde los precios guardados
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {alertas.length>0&&(()=>{
        const porProv={};
        [...alertas].sort((a,b)=>(a.proveedor||"").localeCompare(b.proveedor||"")).forEach(p=>{
          const prov=p.proveedor||"Sin proveedor";
          if(!porProv[prov])porProv[prov]=[];
          porProv[prov].push(p);
        });
        const provOrdenados=Object.keys(porProv).sort();
        return(
          <Card style={{background:"#FF4D6A06",border:"1px solid #FF4D6A22",padding:"16px 20px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{fontSize:12,fontWeight:600,color:G.rojo}}>⚠ Reposicion necesaria — {alertas.length} productos</div>
              <Badge color="rojo">{provOrdenados.length} proveedores</Badge>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {provOrdenados.map(prov=>(
                <div key={prov}>
                  <div style={{fontSize:11,fontWeight:600,color:G.naranja,textTransform:"uppercase",letterSpacing:0.8,marginBottom:6,paddingBottom:4,borderBottom:"1px solid #2A3045"}}>
                    {prov} <span style={{color:G.textoSec,fontWeight:400}}>({porProv[prov].length} productos)</span>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:4}}>
                    {porProv[prov].map(p=>(
                      <div key={p.id} style={{display:"grid",gridTemplateColumns:"90px 1fr 130px 120px",gap:8,alignItems:"center",fontSize:12,padding:"4px 0",borderBottom:"1px solid #2A304522"}}>
                        <span style={{fontFamily:"DM Mono,monospace",fontSize:10,color:G.textoSec}}>{p.codigo}</span>
                        <span style={{fontWeight:500}}>{p.nombre}</span>
                        <span style={{fontFamily:"DM Mono,monospace",color:p.stock===0?G.rojo:G.amarillo,textAlign:"right"}}>
                          {p.stock===0?"AGOTADO":"stock: "+p.stock+" / min "+p.stock_min}
                        </span>
                        <span style={{color:G.textoSec,textAlign:"right"}}>
                          pedir <strong style={{color:G.texto}}>{Math.max(0,p.stock_min*2-p.stock)} u.</strong>
                        </span>
                      </div>
                    ))}
                  </div>
...
// ============================================================
function ModuloAbastecimiento({productos,abastecimiento,onRegistrar,vendedores,proveedores}){
        ...
        <div style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="egresos"        && <ModuloEgresos        egresos={data.egresos} onRegistrar={data.registrarEgreso} onReembolsar={data.marcarReembolsado} vendedores={data.vendedores} proveedores={data.proveedores}/>}
              ...
              {modulo==="abastecimiento" && <ModuloAbastecimiento productos={data.productos} abastecimiento={data.abastecimiento} onRegistrar={data.registrarAbastecimiento} vendedores={data.vendedores} proveedores={data.proveedores}/>}
              {modulo==="configuracion"  && <ModuloConfiguracion  vendedores={data.vendedores} onGuardar={data.guardarVendedor} onToggle={data.toggleVendedor} proveedores={data.proveedores} onGuardProv={data.guardarProveedor} onToggleProv={data.toggleProveedor}/>}
            </>)
          }
        </div>
      </div>
...
// ============================================================
function ModuloConfiguracion({vendedores,onGuardar,onToggle,proveedores,onGuardProv,onToggleProv}){
...
function ModuloProveedores({proveedores,onGuardar,onToggle}){
  ...
  function abrirNuevo(){setEdit(null);setFN("");setFC("");setFT("");setFE("");setFD("");setFNotas("");setModal(true);}
  function abrirEditar(p){setEdit(p);setFN(p.nombre);setFC(p.contacto||"");setFT(p.telefono||"");setFE(p.email||"");setFD(p.direccion||"");setFNotas(p.notas||"");setModal(true);}

  async function guardar(){
    if(!fNombre.trim())return;
    setLoad(true);
    await onGuardar({nombre:fNombre.trim(),contacto:fContacto,telefono:fTel,email:fEmail,direccion:fDir,notas:fNotas,activo:true},editando?.id||null);
    setLoad(false);setModal(false);
  }
