...
function ModuloCaja({ventas,egresos,toast}){
  ...
  // Egresos: solo los que efectivamente salieron de la caja del local (pagador="Pensok")
  // y que no están pendientes de reembolso (si pagador="Pensok" y reembolso_pendiente=true,
  // significa que en realidad todavía no fue pagado por el local, sino que se debe a alguien)
  const egresosDesde = egresos.filter(e=>
    e.fecha >= fechaInicio &&
    !TIPOS_EGRESO_EXCLUIDOS.includes(e.tipo) &&
    e.pagador==="Pensok" &&
    !(e.reembolso_pendiente && !e.reembolsado)
  );
  // Los pagos con referencia_id apuntan a una venta puntual (cobro de deuda registrado al editar la venta).
  // Esa plata YA se cuenta a través de ventasDesde (venta.cobrado / monto_cobrado), así que acá se excluye
  // para no sumarla dos veces. Solo quedan los pagos de deuda "sueltos" (sin venta asociada en el sistema).
  const pagosDesde = pagosDia.filter(p=>p.fecha >= fechaInicio && !p.referencia_id);
  const movsDesde = movimientos.filter(m=>m.fecha >= fechaInicio);

  // Monto efectivamente cobrado de una venta (soporta pagos parciales)
  const cobradoDeVenta = (v) => {
    // Si la venta está completamente cobrada, siempre es el total —
    // monto_cobrado puede haber quedado desactualizado de un pago parcial previo a completarse.
    if(v.cobrado) return v.total||0;
    // Si no está completamente cobrada pero tiene un pago parcial registrado, usar ese monto
    if((v.monto_cobrado||0) > 0) return v.monto_cobrado;
    return 0;
  };
  ...
  const calcBolsillo = (bolsillo) => {
    ...
    if(bolsillo==="caja_chica"){
      ...
      const vEf   = ventasDesde.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+cobradoDeVenta(v),0)+cobEf;
    ...
    if(bolsillo==="mp"){
      ...
      const vMP_  = ventasDesde.filter(v=>MP_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+cobradoDeVenta(v),0)+cobMP;
      const gMP   = egresosDesde.filter(e=>MP_METODOS.includes(e.metodo_pago)).reduce((s,e)=>s+(e.monto||0),0)+reMP;
      return vMP_ - gMP + movNeto;
    }
    if(bolsillo==="banco"){
      ...
      const vBa   = ventasDesde.filter(v=>BANCO_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+cobradoDeVenta(v),0)+cobBa;
      const gBa   = egresosDesde.filter(e=>BANCO_METODOS.includes(e.metodo_pago)).reduce((s,e)=>s+(e.monto||0),0)+reBa;
      return vBa - gBa + movNeto;
    }
    if(bolsillo==="ahorro"){
      return movNeto; // ahorro solo se mueve por traspasos manuales entre bolsillos
    }
    return 0;
  };
  ...
  // Totales informativos del período completo (para mostrar desglose)
  const vEfectivo = ventasDesde.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+cobradoDeVenta(v),0);
  const vMP       = ventasDesde.filter(v=>MP_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+cobradoDeVenta(v),0);
  const vBanco    = ventasDesde.filter(v=>BANCO_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+cobradoDeVenta(v),0);
  const vCC       = ventasDesde.filter(v=>v.metodo_pago==="Cuenta corriente").reduce((s,v)=>s+cobradoDeVenta(v),0);
  ...
  const gOtros    = egresosDesde.reduce((s,e)=>s+(e.monto||0),0) - gEfectivo;
  const gTotal    = egresosDesde.reduce((s,e)=>s+(e.monto||0),0);
  const ganNeta   = vTotal - gTotal;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <div style={{flex:1}}/>
        <span style={{fontSize:11,color:G.textoSec}}>Contando desde {fechaInicio}</span>
      ...
      {tab==="nuevo"&&(<>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
            <div>
              ...
              <div style={{fontSize:11,color:G.textoSec,marginTop:2}}>
                ...
                {ultimoCierre&&<> · último cierre guardado: {ultimoCierre.fecha}</>}
              </div>
            </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <Card style={{background:G.sup2,border:`1px solid ${G.borde}`}}>
              <ST>Saldo de arranque — configurado el {fechaInicio}</ST>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[
                    {l:"Caja Chica",  esp:esperadoCaja,   real:realCaja,   diff:diffCaja,
                      det:`Arranque ${fmt(arranqueCaja)} + ventas ef. ${fmt(vEfectivo-cobDeudaEf)}${cobDeudaEf>0?` + cobros deuda ef. ${fmt(cobDeudaEf)}`:""} − gastos ef. ${fmt(gEfectivo-reembDeudaEf)}${reembDeudaEf>0?` − reembolsos ef. ${fmt(reembDeudaEf)}`:""}${movNetoCaja!==0?` ${movNetoCaja>0?"+":"−"} movimientos ${fmt(Math.abs(movNetoCaja))}`:""}`},
                    {l:"Mercado Pago",esp:esperadoMP,     real:realMP,     diff:diffMP,
                      det:`Arranque ${fmt(arranqueMP)} + ventas MP ${fmt(vMP-cobDeudaMP)}${cobDeudaMP>0?` + cobros deuda MP ${fmt(cobDeudaMP)}`:""}${reembDeudaMP>0?` − reembolsos MP ${fmt(reembDeudaMP)}`:""}${movNetoMP!==0?` ${movNetoMP>0?"+":"−"} movimientos ${fmt(Math.abs(movNetoMP))}`:""}`},
                    {l:"Banco",       esp:esperadoBanco,  real:realBanco,  diff:diffBanco,
                      det:`Arranque ${fmt(arranqueBanco)} + ventas banco ${fmt(vBanco-cobDeudaBanco)}${cobDeudaBanco>0?` + cobros deuda banco ${fmt(cobDeudaBanco)}`:""}${reembDeudaBanco>0?` − reembolsos banco ${fmt(reembDeudaBanco)}`:""}${movNetoBanco!==0?` ${movNetoBanco>0?"+":"−"} movimientos ${fmt(Math.abs(movNetoBanco))}`:""}`},
