...
function ModuloProductos({productos,onGuardar,proveedores,esAdmin=true}){
  ...
  const [catsFiltro, setCatsFiltro] = useState([]); // categorias seleccionadas para PDF
      ...
      {modalLista&&(
        <Modal title="Exportar lista de precios" onClose={()=>setModalLista(false)}
          footer={<><Btn variant="secondary" onClick={()=>setModalLista(false)}>Cancelar</Btn><Btn disabled={generando} onClick={()=>{
              ...
              const prodsExportar=catsFiltro.length>0?base.filter(p=>catsFiltro.includes(p.categoria)):base;
              generarListaPDF(tipoLista,prodsExportar);
