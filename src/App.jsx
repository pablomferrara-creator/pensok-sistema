...
function ModuloVenta({clientes,productos,onRegistrar,vendedores}){
  ...
  async function cerrarVenta(){
    ...
    setTimeout(()=>{setItems([]);setDescuento("10");setMetodo("Efectivo");setClienteId("");setOk(false);},2000);
  }
...
// MODULO: INGRESOS
// ============================================================
function ModuloIngresos({ventas,vendedores,productos,onEditar,onEliminar,totalVentas,filtroInicial=""}){
...
function ModuloEgresos({egresos,onRegistrar,onReembolsar,vendedores,proveedores,onEditar,onEliminar}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        <MetricCard label="Pagado este mes"  value={fmt(totalMesPagado)} color={G.rojo} sub={`Total devengado: ${fmt(totalMes)}`}/>
...
// MODULO: PRODUCTOS
// ============================================================
function ModuloProductos({productos,onGuardar,proveedores,granel}){
  ...
  const [fGranelId,   setFGranelId]   = useState("");
  const [fConsumoGran,setFConsumoGran]= useState("");
  ...
  async function generarListaPDF(tipo, productos_filtrados){
    ...
    dibujarEncabezado();
    let y = 44;
    const COL_PROD=14, COL_CAT=140, COL_PRECIO=172;
  ...
  function abrirEditar(p){
    ...
    setFGranelId(String(p.granel_id||""));
    setFConsumoGran(String(p.consumo_granel||""));
    setModal(true);
  }
  function abrirNuevo(){
    setEditando(null);setFK("");setFN("");setFCat(CATEGORIAS[0]);setFMon("ARS");
    setFCosto("");setFGanMin("");setFGanMay("");setFStock("");setFStockMin("");
    setFProv("");setFIva("21");setFDescProv("0");setFGranelId("");setFConsumoGran("");setModal(true);
  }
  ...
  async function guardar(){
    ...
    const datos={
      ...
      consumo_granel:parseFloat(fConsumoGran)||0,
    };
    await onGuardar(datos,editando?.id||null);
    setLoading(false);setModal(false);
  }
...
function ModuloAbastecimiento({productos,abastecimiento,onRegistrar,vendedores,proveedores,onEditar,onEliminar}){
  ...
  const [modalGranel,  setModalGranel] = useState(false); // nuevo/editar granel
  const [editGranel,   setEditGranel]  = useState(null);
  const [gNombre,      setGNombre]     = useState("");
  const [gUnidad,      setGUnidad]     = useState("litros");
  const [gStock,       setGStock]      = useState("");
  const [gStockMin,    setGStockMin]   = useState("");
  const [gLoading,     setGLoading]    = useState(false);
  // envasado
  const [modalEnvasar, setModalEnvasar]= useState(false);
  const [envGranel,    setEnvGranel]   = useState(null);
  const [envProdBusq,  setEnvProdBusq] = useState("");
  const [envProd,      setEnvProd]     = useState(null);
  const [envCant,      setEnvCant]     = useState("");
  const [envResp,      setEnvResp]     = useState("");
  const [envNotas,     setEnvNotas]    = useState("");
  const [envLoading,   setEnvLoading]  = useState(false);

  useEffect(()=>{ cargarGranel(); },[]);

  async function cargarGranel(){
    const [{data:g},{data:e}] = await Promise.all([
      supabase.from("productos_granel").select("*").order("nombre"),
      supabase.from("envasados").select("*").order("created_at",{ascending:false}).limit(100),
    ]);
    setGranel(g||[]);
    setEnvasados(e||[]);
  }

  async function guardarGranel(){
    if(!gNombre) return; setGLoading(true);
    const datos={nombre:gNombre,unidad:gUnidad,stock_actual:parseFloat(gStock)||0,stock_minimo:parseFloat(gStockMin)||0,activo:true};
    if(editGranel) await supabase.from("productos_granel").update(datos).eq("id",editGranel.id);
    else await supabase.from("productos_granel").insert(datos);
    setGLoading(false); setModalGranel(false);
    await cargarGranel();
  }

  function abrirNuevoGranel(){
    setEditGranel(null);setGNombre("");setGUnidad("litros");setGStock("");setGStockMin("");setModalGranel(true);
  }
  function abrirEditarGranel(g){
    setEditGranel(g);setGNombre(g.nombre);setGUnidad(g.unidad);setGStock(String(g.stock_actual));setGStockMin(String(g.stock_minimo||""));setModalGranel(true);
  }

  // Calcular consumo granel al envasar
  const consumoCalculado = envProd && envCant
    ? (parseFloat(envProd.consumo_granel)||0) * (parseFloat(envCant)||0)
    : 0;

  async function registrarEnvasado(){
    if(!envGranel||!envProd||!envCant||!envResp){return;}
    setEnvLoading(true);
    // Insertar envasado
    await supabase.from("envasados").insert({
      fecha: hoy(),
      granel_id: envGranel.id,
      producto_id: envProd.id,
      cantidad_envasada: parseInt(envCant),
      consumo_granel: consumoCalculado,
      responsable: envResp,
      notas: envNotas,
    });
    // Descontar del granel
    await supabase.from("productos_granel")
      .update({stock_actual: envGranel.stock_actual - consumoCalculado})
      .eq("id", envGranel.id);
    // Sumar stock al producto envasado
    const prodActual = productos.find(p=>p.id===envProd.id);
    if(prodActual){
      await supabase.from("productos")
        .update({stock: (prodActual.stock||0) + parseInt(envCant)})
        .eq("id", envProd.id);
    }
    setEnvLoading(false); setModalEnvasar(false);
    setEnvProd(null);setEnvProdBusq("");setEnvCant("");setEnvResp("");setEnvNotas("");
    await cargarGranel();
  }

  const envProdFilt = useMemo(()=>{
    if(!envProdBusq) return [];
    const q=envProdBusq.toLowerCase();
    return productos.filter(p=>p.activo&&p.granel_id===envGranel?.id&&(p.nombre.toLowerCase().includes(q)||p.codigo?.toLowerCase().includes(q)));
  },[envProdBusq,productos,envGranel]);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        <Btn variant={vista==="granel"?"primary":"secondary"}    onClick={()=>setV("granel")}>🧴 Granel & Envasado</Btn>
      </div>
      {vista==="nuevo"&&(
      ...
      {vista==="granel"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <ST>Productos Granel</ST>
            <Btn small onClick={abrirNuevoGranel}>+ Nuevo granel</Btn>
          </div>
          {granel.length===0&&<div style={{textAlign:"center",padding:"32px 0",color:G.textoSec}}>No hay productos granel cargados</div>}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
            {granel.map(g=>{
              const pct = g.stock_minimo>0 ? Math.min(g.stock_actual/g.stock_minimo*50,100) : 100;
              const color = g.stock_actual<=0?G.rojo:g.stock_minimo>0&&g.stock_actual<=g.stock_minimo?G.amarillo:G.verde;
              const histG = envasados.filter(e=>e.granel_id===g.id);
              return(
                <Card key={g.id} style={{border:`1px solid ${color}33`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:14}}>{g.nombre}</div>
                      <div style={{fontSize:11,color:G.textoSec,marginTop:2}}>Unidad: {g.unidad}</div>
                    </div>
                    <div style={{display:"flex",gap:6}}>
                      <button onClick={()=>abrirEditarGranel(g)} style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:6,padding:"4px 10px",fontSize:11,cursor:"pointer",color:G.texto}}>✏ Editar</button>
                      <button onClick={()=>{setEnvGranel(g);setModalEnvasar(true);}} style={{background:"#00C48C22",border:`1px solid ${G.verde}44`,borderRadius:6,padding:"4px 10px",fontSize:11,cursor:"pointer",color:G.verde}}>🧴 Envasar</button>
                    </div>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontSize:12,color:G.textoSec}}>Stock actual</span>
                    <span style={{fontFamily:"DM Mono,monospace",fontWeight:700,color,fontSize:16}}>{g.stock_actual} {g.unidad}</span>
                  </div>
                  {g.stock_minimo>0&&(
                    <div style={{marginBottom:8}}>
                      <div style={{height:6,background:G.sup2,borderRadius:3,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${pct}%`,background:color,borderRadius:3,transition:"width .3s"}}/>
                      </div>
                      <div style={{fontSize:10,color:G.textoSec,marginTop:3}}>Mínimo: {g.stock_minimo} {g.unidad}</div>
                    </div>
                  )}
                  {histG.length>0&&(
                    <div style={{borderTop:`1px solid ${G.borde}22`,paddingTop:8,marginTop:4}}>
                      <div style={{fontSize:10,color:G.textoSec,fontWeight:600,marginBottom:4}}>ÚLTIMOS ENVASADOS</div>
                      {histG.slice(0,3).map(e=>(
                        <div key={e.id} style={{display:"flex",justifyContent:"space-between",fontSize:11,padding:"2px 0",color:G.textoSec}}>
                          <span>{e.fecha} · {e.responsable}</span>
                          <span style={{color:G.rojo}}>−{e.consumo_granel} {g.unidad}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {/* Historial envasados */}
          <ST style={{marginTop:8}}>Historial de envasados</ST>
          {envasados.length===0&&<div style={{textAlign:"center",padding:"24px 0",color:G.textoSec}}>Sin registros</div>}
          {envasados.slice(0,30).map(e=>{
            const g=granel.find(x=>x.id===e.granel_id);
            return(
              <Card key={e.id} style={{padding:"10px 16px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:600}}>{e.fecha} — {g?.nombre||"Granel"}</div>
                    <div style={{fontSize:12,color:G.textoSec,marginTop:2}}>
                      {e.cantidad_envasada} unidades envasadas · Consumo: <strong style={{color:G.rojo}}>{e.consumo_granel} {g?.unidad||""}</strong>
                    </div>
                    {e.notas&&<div style={{fontSize:11,color:G.textoSec,fontStyle:"italic",marginTop:2}}>"{e.notas}"</div>}
                  </div>
                  <Badge color="azul">{e.responsable}</Badge>
                </div>
              </Card>
            );
          })}
      ...
      {modalGranel&&(
        <Modal title={editGranel?"Editar producto granel":"Nuevo producto granel"} onClose={()=>setModalGranel(false)}
          footer={<><Btn variant="secondary" onClick={()=>setModalGranel(false)}>Cancelar</Btn><Btn disabled={!gNombre||gLoading} onClick={guardarGranel}>{gLoading?<span style={{display:"flex",alignItems:"center",gap:6}}><Spinner/>Guardando</span>:"Guardar"}</Btn></>}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Fi label="Nombre del producto granel" value={gNombre} onChange={setGNombre} placeholder="Ej: Cloro Líquido Granel"/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Unidad de medida" value={gUnidad} onChange={setGUnidad} options={["litros","kg","unidades"]}/>
              <Fi label="Stock mínimo (alerta)" value={gStockMin} onChange={setGStockMin} type="number" placeholder="0"/>
            </div>
            <Fi label={`Stock actual (${gUnidad})`} value={gStock} onChange={setGStock} type="number" placeholder="0"/>
          </div>
        </Modal>
      )}

      {/* Modal envasar */}
      {modalEnvasar&&envGranel&&(
        <Modal title={`Envasar desde: ${envGranel.nombre}`} onClose={()=>{setModalEnvasar(false);setEnvProd(null);setEnvProdBusq("");setEnvCant("");}}
          footer={<><Btn variant="secondary" onClick={()=>setModalEnvasar(false)}>Cancelar</Btn><Btn disabled={!envProd||!envCant||!envResp||envLoading} onClick={registrarEnvasado}>{envLoading?<span style={{display:"flex",alignItems:"center",gap:6}}><Spinner/>Guardando</span>:"Registrar envasado"}</Btn></>}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            {/* Stock disponible */}
            <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between"}}>
              <span style={{fontSize:12,color:G.textoSec}}>Stock disponible</span>
              <span style={{fontFamily:"DM Mono,monospace",fontWeight:700,color:G.verde}}>{envGranel.stock_actual} {envGranel.unidad}</span>
            </div>
            {/* Buscar producto envasado vinculado */}
            <div>
              <div style={{fontSize:11,color:G.textoSec,fontWeight:600,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}}>Producto a envasar</div>
              <div style={{position:"relative"}}>
                <input value={envProd?envProd.nombre:envProdBusq} onChange={e=>{setEnvProd(null);setEnvProdBusq(e.target.value);}}
                  placeholder="Buscar producto envasado vinculado..."
                  style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:8,padding:"9px 12px",color:G.texto,fontSize:13,width:"100%",outline:"none"}}/>
                {!envProd&&envProdBusq.length>1&&(()=>{
                  const hits = productos.filter(p=>p.activo&&(p.nombre.toLowerCase().includes(envProdBusq.toLowerCase())));
                  if(!hits.length) return <div style={{position:"absolute",top:"100%",left:0,right:0,background:G.sup,border:`1px solid ${G.borde}`,borderRadius:8,zIndex:50,padding:"10px 14px",fontSize:12,color:G.textoSec}}>Sin resultados</div>;
                  return(
                    <div style={{position:"absolute",top:"100%",left:0,right:0,background:G.sup,border:`1px solid ${G.borde}`,borderRadius:8,zIndex:50,maxHeight:200,overflowY:"auto"}}>
                      {hits.map(p=>(
                        <div key={p.id} onClick={()=>{setEnvProd(p);setEnvProdBusq("");}}
                          style={{padding:"9px 14px",cursor:"pointer",borderBottom:`1px solid ${G.borde}22`,fontSize:12}}
                          onMouseEnter={e=>e.currentTarget.style.background=G.sup2}
                          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <div style={{fontWeight:500}}>{p.nombre}</div>
                          <div style={{fontSize:11,color:G.textoSec}}>
                            Stock: {p.stock} · Consumo por unidad: {p.consumo_granel||"⚠ no configurado"} {envGranel.unidad}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {envProd&&!envProd.consumo_granel&&(
                <div style={{fontSize:12,color:G.amarillo,marginTop:6}}>⚠ Este producto no tiene configurado el consumo de granel. Editalo en Productos para que el descuento sea automático.</div>
              )}
            </div>
            <Fi label="Cantidad a envasar (unidades)" value={envCant} onChange={setEnvCant} type="number" placeholder="0"/>
            {consumoCalculado>0&&(
              <div style={{background:"#FF4D6A11",border:"1px solid #FF4D6A33",borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between"}}>
                <span style={{fontSize:12,color:G.textoSec}}>Se descontará del granel</span>
                <span style={{fontFamily:"DM Mono,monospace",fontWeight:700,color:G.rojo}}>{consumoCalculado} {envGranel.unidad}</span>
              </div>
            )}
            {consumoCalculado>envGranel.stock_actual&&(
              <div style={{fontSize:12,color:G.rojo,fontWeight:600}}>⚠ Stock insuficiente en granel ({envGranel.stock_actual} {envGranel.unidad} disponibles)</div>
            )}
            <Fi label="Responsable" value={envResp} onChange={setEnvResp} options={(vendedores||[]).map(v=>v.nombre)}/>
            <Fi label="Notas (opcional)" value={envNotas} onChange={setEnvNotas} placeholder="Observaciones..."/>
          </div>
        </Modal>
      )}
...
export default function App(){
  ...
  const tabs=[
    ...
    {id:"caja",           label:"Cierre de Caja", alerta:0},
        ...
        <div style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="ingresos"       && <ModuloIngresos       ventas={data.ventasConItems} vendedores={data.vendedores} productos={data.productos} onEditar={data.editarVenta} onEliminar={data.eliminarVenta} totalVentas={data.totalVentas} filtroInicial={filtroIngresos}/>}
              ...
              {modulo==="productos"      && <ModuloProductos      productos={data.productos} onGuardar={data.guardarProducto} proveedores={data.proveedores} granel={data.granelProductos||[]}/>}
