        ...
        <Modal title="Registrar egreso" onClose={()=>setModal(false)}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <Fi label="Quien pago?" value={fPagador} onChange={setFPag}  options={PAGADORES}/>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {alertas.length>0&&(
        <Card style={{background:"#FF4D6A06",border:`1px solid #FF4D6A22`,padding:"12px 18px"}}>
          <div style={{fontSize:12,fontWeight:600,color:G.rojo,marginBottom:8}}>⚠ Reposicion necesaria</div>
      ...
      <div style={{background:G.sup,border:`1px solid ${G.borde}`,borderRadius:12,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{borderBottom:`1px solid ${G.borde}`}}>{["Codigo","Producto","Cat.","Stock","Costo","Minorista","Especial","Mayorista","Vendidos","Estado",""].map(h=><th key={h} style={{padding:"10px 12px",textAlign:"left",color:G.textoSec,fontWeight:500,fontSize:11,textTransform:"uppercase",letterSpacing:0.5,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
            <tbody>
              {filtrados.map((p,i)=>{
      ...
      {modal&&(
        <Modal title={editando?"Editar producto":"Nuevo producto"} onClose={()=>setModal(false)} maxWidth={540}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Fi label="Nombre" value={fNombre} onChange={setFN} placeholder="Ej: Cloro liquido 5L"/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Proveedor" value={fProv}  onChange={setFProv} options={PROVEEDORES}/>
