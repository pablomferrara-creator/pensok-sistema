    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {(()=>{
        ...
        if(periodo==="hoy"||periodo==="dia"){
          ...
          for(let m=1;m<=12;m++){
            const mesStr=anioRef+"-"+String(m).padStart(2,"0");
        ...
        // ── Dimensiones SVG ──
        const W=900, H=220, PL=64, PR=20, PT=20, PB=40;
        const cW=W-PL-PR, cH=H-PT-PB;
        const maxVal=Math.max(...puntos.map(p=>p.valor), objetivo.valor*1.1, 1);
        const meta = objetivo.sinDatos ? 0 : objetivo.valor;

        // Coordenadas
        const xPos=(i)=>PL + (i/(puntos.length-1||1))*cW;
        const yPos=(v)=>PT + cH - (v/maxVal)*cH;
        const metaY = meta>0 ? yPos(meta) : -1;

        // Construir polyline points
        const pts = puntos.map((p,i)=>`${xPos(i)},${yPos(p.valor)}`).join(" ");

        // Área de relleno con gradiente: debajo de meta=rojo, zona entre meta y punto=verde
        // Usamos clipPath para cada zona
        const areaAbajo = puntos.map((p,i)=>{
          const x=xPos(i); const y=yPos(p.valor);
          return `${x},${y}`;
        }).join(" ") + ` ${xPos(puntos.length-1)},${PT+cH} ${xPos(0)},${PT+cH}`;

        // Líneas de grilla Y
        const gridVals=[0,0.25,0.5,0.75,1].map(f=>Math.round(maxVal*f));

        // Tooltip state — usamos solo SVG/title por ahora
        const labelPer = periodo==="hoy"||periodo==="dia" ? "hora" : periodo==="anio" ? "mes" : "día";

        return(
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <ST style={{margin:0}}>Evolución de facturación — {labelPeriodo}</ST>
              <div style={{display:"flex",gap:14,fontSize:11,color:G.textoSec}}>
                {meta>0&&<span style={{color:"#FF4D6A"}}>┄ Objetivo: {fmt(meta)}</span>}
            ...
            <div style={{overflowX:"auto"}}>
              <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",minWidth:320,height:H,display:"block"}}>
                <defs>
                  <clipPath id="clipAboveMeta">
                    <rect x={PL} y={PT} width={cW} height={metaY>0?metaY-PT:0}/>
                  </clipPath>
                  <clipPath id="clipBelowMeta">
                    <rect x={PL} y={metaY>0?metaY:PT} width={cW} height={metaY>0?PT+cH-metaY:cH}/>
                  </clipPath>
                ...
                {gridVals.map((v,i)=>(
                  <g key={i}>
                    <line x1={PL} y1={yPos(v)} x2={W-PR} y2={yPos(v)} stroke="#2A3045" strokeWidth="1"/>
                    <text x={PL-6} y={yPos(v)+4} fontSize="9" fill="#7A8099" textAnchor="end">
                      {v>=1000000?"$"+(v/1000000).toFixed(1)+"M":v>=1000?"$"+(v/1000).toFixed(0)+"k":"$"+v}
                    </text>
                  </g>
                ))}

                {/* Área verde (por encima de meta) */}
                {meta>0&&(
                  <polygon points={areaAbajo} fill="url(#gradVerde)" clipPath="url(#clipAboveMeta)"/>
                )}

                {/* Área roja (por debajo de meta) */}
                {meta>0
                  ? <polygon points={areaAbajo} fill="url(#gradRojo)" clipPath="url(#clipBelowMeta)"/>
                  : <polygon points={areaAbajo} fill="url(#gradVerde)"/>
                }

                {/* Línea de objetivo punteada */}
                {meta>0&&metaY>0&&(
                  <line x1={PL} y1={metaY} x2={W-PR} y2={metaY}
                    stroke="#FF4D6A" strokeWidth="1.5" strokeDasharray="6,4"/>
                )}

                {/* Línea de facturación */}
                <polyline points={pts} fill="none" stroke={G.verde} strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round"/>

                {/* Puntos y tooltips */}
                {puntos.map((p,i)=>{
                  const x=xPos(i); const y=yPos(p.valor);
                  const color=meta>0?(p.valor>=meta?G.verde:p.valor>=meta*0.7?G.amarillo:G.rojo):G.verde;
                  return(
                    <g key={i}>
                      <circle cx={x} cy={y} r="4" fill={color} stroke={G.fondo} strokeWidth="1.5">
                        <title>{p.label}: {fmt(p.valor)}{meta>0?" | Objetivo: "+fmt(meta):""}</title>
                      </circle>
                    </g>
                  );
                })}

                {/* Labels eje X — mostrar subset para no solapar */}
                {puntos.map((p,i)=>{
                  const step = puntos.length>20?5:puntos.length>10?2:1;
                  if(i%step!==0&&i!==puntos.length-1) return null;
                  return(
                    <text key={i} x={xPos(i)} y={H-8} fontSize="9" fill="#7A8099" textAnchor="middle">{p.label}</text>
                  );
                })}
              </svg>
            </div>
...
function ModuloCaja({ventas,egresos,toast}){
  ...
  const cobrosDeuda = pagosDiaFecha.filter(p=>p.tipo==="ingreso");
  const reembolsosDia = pagosDiaFecha.filter(p=>p.tipo==="egreso");
  // Pagos de deuda por billetera
  ...
  const cobrosEfectivo = cobrosDeuda.filter(p=>p.metodo_pago==="Efectivo").reduce((s,p)=>s+p.monto,0);
  const cobrosMP       = cobrosDeuda.filter(p=>MP_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
  const cobrosBanco    = cobrosDeuda.filter(p=>BANCO_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
  const reembEfectivo  = reembolsosDia.filter(p=>p.metodo_pago==="Efectivo").reduce((s,p)=>s+p.monto,0);
  const reembMP        = reembolsosDia.filter(p=>MP_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
  const reembBanco     = reembolsosDia.filter(p=>BANCO_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
  ...
  const vEfectivo  = ventasDia.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+(v.total||0),0) + cobrosEfectivo;
  const vMP        = ventasDia.filter(v=>MP_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0) + cobrosMP;
  const vBanco     = ventasDia.filter(v=>BANCO_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0) + cobrosBanco;
  ...
  const gEfectivo  = egresosDia.filter(e=>e.metodo_pago==="Efectivo").reduce((s,e)=>s+(e.monto||0),0) + reembEfectivo;
