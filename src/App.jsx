...
const TIPOS_EGRESO = ["Gasto fijo", "Gasto variable", "Retiro de capital"];
        ...
        <Card>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Btn full disabled={!email||(modo!=="recuperar"&&!pass)||loading} onClick={handleSubmit}>
              {loading?<span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><Spinner/> Procesando...</span>:modo==="login"?"Ingresar":modo==="registro"?"Crear cuenta":"Enviar email de recuperacion"}
            </Btn>
            {modo==="login"&&(
            ...
            <div style={{textAlign:"center",fontSize:12,color:G.textoSec,paddingTop:modo==="login"?0:4}}>
              {modo==="login"?"Primera vez? ":modo==="registro"?"Ya tenes cuenta? ":"Recordaste tu contrasena? "}
              <span style={{color:G.verde,cursor:"pointer"}} onClick={()=>{setModo(modo==="login"?"registro":"login");setError("");}}>
...
function ModuloIngresos({ventas,vendedores,productos,clientes,onEditar,onEliminar,totalVentas,filtroInicial="",esAdmin=true}){
  ...
  async function imprimirTicket(v){
    ...
    const doc = new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
    const W=210,H=297;
    const azul=[20,53,107],azulClaro=[41,98,180],gris=[100,100,100],negro=[30,30,30],blanco=[255,255,255],rojo=[180,30,30],verde=[0,150,100];
    const fmtP = n => '$ '+Math.round(n||0).toLocaleString('es-AR');

    // ── HEADER ──
    doc.setFillColor(...azul);
    doc.rect(0,0,W,42,'F');
    // Icono P en lugar de logo
    doc.setFillColor(0,196,140);
    doc.roundedRect(8,6,28,28,4,4,'F');
    doc.setFont('helvetica','bold');
    doc.setFontSize(18);
    doc.setTextColor(0,0,0);
    doc.text('P',22,25,{align:'center'});
    doc.setTextColor(...blanco);
    doc.setFont('helvetica','bold');
    doc.setFontSize(20);
    ...
    doc.setFont('helvetica','normal');
    doc.setFontSize(8.5);
    doc.text('DOMOKIP SAS',44,22);
    doc.text('Gelves 1126, B1629 Pilar, Buenos Aires',44,27);
    doc.text('CUIT: 30-71686952-7',44,32);
    doc.text('Tel: 11-7064-5115  ·  @pensok.piletas',44,37);
  ...
  const redondear100_ing = n => Math.ceil(n/100)*100;
  ...
  function agregarItemEv(prod){
    setEvItems(prev=>[...prev,{nombre:prod.nombre,cantidad:"1",precio:String(prod.precio_min),costo:prod.costo||0}]);
    setEvBusqueda("");
  }
  async function guardarVenta(){
    if(!editandoV)return; setEvLoading(true);
    ...
    const bruto=itemsNum.reduce((s,i)=>s+i.precio*i.cantidad,0);
    const desc=editandoV.descuento||0;
    const total=redondear100_ing(bruto*(1-desc/100));
    const costos=itemsNum.reduce((s,i)=>s+(i.costo||0)*i.cantidad,0);
    const comision=parseFloat(evComision)||0;
    const ganancia=total-costos-comision;
    await onEditar(editandoV.id,{cliente_nombre:evCliente,vendedor:evVendedor,metodo_pago:evMetodo,cobrado:evCobrado,entregado:evEntregado,comision_plataforma:comision,total,ganancia});
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                  ...
                  {v.metodo_pago!=="Efectivo"&&!(v.comision_plataforma>0)&&(
                    <span style={{background:"#FFB80022",color:G.amarillo,border:"1px solid #FFB80055",borderRadius:6,padding:"2px 9px",fontSize:11,fontWeight:600}}>⚠ Sin comision</span>
                  )}
                  {v.metodo_pago!=="Efectivo"&&v.comision_plataforma>0&&(
                    <Badge color="gris">Comision {fmt(v.comision_plataforma)}</Badge>
                  )}
                </div>
                {(v.items||[]).length>0&&(
                  <div style={{marginTop:8,fontSize:11,color:G.textoSec}}>
                    {(v.items||[]).map((it,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}>
                        <span>{it.nombre} <strong style={{color:G.texto}}>x{it.cantidad}</strong></span>
      ...
      {quickEditV&&(
        <Modal title="Actualizar venta" onClose={()=>setQuickEditV(null)}
          footer={<><Btn variant="secondary" onClick={()=>setQuickEditV(null)}>Cancelar</Btn><Btn disabled={qeLoading} onClick={guardarQuickEdit}>{qeLoading?"Guardando...":"Guardar"}</Btn></>}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.textoSec}}>
            ...
            <div style={{display:"flex",gap:20}}>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}><input type="checkbox" checked={qeCobrado} onChange={e=>setQeCobrado(e.target.checked)}/> Cobrado</label>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}><input type="checkbox" checked={qeEntregado} onChange={e=>setQeEntregado(e.target.checked)}/> Entregado</label>
            </div>
            {quickEditV.metodo_pago!=="Efectivo"&&(
              <Fi label="Comision plataforma ($)" value={qeComision} onChange={setQeComision} type="number" placeholder="0"/>
            )}
          </div>
        </Modal>
      )}
      ...
      {editandoV&&(()=>{
        ...
        const costos=itemsNum.reduce((s,i)=>s+(i.costo||0)*i.cantidad,0);
        const comision=parseFloat(evComision)||0;
        const ganPreview=total-costos-comision;
        const conCom=METODOS_CON_COMISION_ING.includes(evMetodo);
        return(
          <Modal title="Editar venta" onClose={()=>setEditandoV(null)} maxWidth={640}
            footer={<><Btn variant="secondary" onClick={()=>setEditandoV(null)}>Cancelar</Btn><Btn disabled={evLoading} onClick={guardarVenta}>{evLoading?"Guardando...":"Guardar cambios"}</Btn></>}>
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.textoSec,display:"flex",justifyContent:"space-between"}}>
                ...
                <span style={{color:G.verde,fontFamily:"DM Mono,monospace",fontWeight:600}}>{fmt(total)}</span>
              </div>
              <Fi label="Cliente" value={evCliente} onChange={setEvCliente} options={["CONSUMIDOR FINAL",...(clientes||[]).filter(c=>c.activo).map(c=>c.nombre)]}/>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <Fi label="Vendedor" value={evVendedor} onChange={setEvVendedor} options={(vendedores||[]).map(v=>v.nombre)}/>
                <Fi label="Metodo de pago" value={evMetodo} onChange={v=>{setEvMetodo(v);if(!METODOS_CON_COMISION_ING.includes(v))setEvComision("0");}} options={METODOS_PAGO}/>
              </div>
              {conCom&&(
                <div style={{background:"#4D9EFF11",border:"1px solid #4D9EFF33",borderRadius:8,padding:"10px 14px"}}>
                  <Fi label="Comision plataforma ($)" value={evComision} onChange={setEvComision} type="number" placeholder="0"/>
                  {comision>0&&<div style={{fontSize:11,color:G.textoSec,marginTop:4}}>Ganancia: <strong style={{color:G.verde}}>{fmt(ganPreview)}</strong></div>}
                </div>
              )}
              <div style={{display:"flex",gap:20}}>
                <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}><input type="checkbox" checked={evCobrado} onChange={e=>setEvCobrado(e.target.checked)}/> Cobrado</label>
                <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}><input type="checkbox" checked={evEntregado} onChange={e=>setEvEntregado(e.target.checked)}/> Entregado</label>
              </div>
              <ST>Productos</ST>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {evItems.map((item,idx)=>(
                  <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 32px",gap:8,alignItems:"center"}}>
                    <div style={{background:G.sup2,border:"1px solid "+G.borde,borderRadius:7,padding:"7px 10px",fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.nombre}</div>
                    <input type="number" min="1" value={item.cantidad} onChange={e=>actualizarItem(idx,"cantidad",e.target.value)} style={{background:G.sup2,border:"1px solid "+G.borde,borderRadius:7,padding:"7px 8px",color:G.texto,fontSize:12,outline:"none",textAlign:"center",width:"100%"}}/>
                    <input type="number" min="0" value={item.precio} onChange={e=>actualizarItem(idx,"precio",e.target.value)} style={{background:G.sup2,border:"1px solid "+G.borde,borderRadius:7,padding:"7px 8px",color:G.texto,fontSize:12,outline:"none",textAlign:"right",width:"100%"}}/>
                    <button onClick={()=>eliminarItemEv(idx)} style={{background:"#FF4D6A18",border:"1px solid #FF4D6A33",borderRadius:7,color:G.rojo,cursor:"pointer",fontSize:14,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center"}}>x</button>
                  </div>
                ))}
                <div style={{position:"relative"}}>
                  <input value={evBusqueda} onChange={e=>setEvBusqueda(e.target.value)} placeholder="+ Buscar producto..."
                  ...
                  {evBusqueda.length>1&&(()=>{
                    ...
                    return(<div style={{position:"absolute",top:"100%",left:0,right:0,background:G.sup,border:"1px solid "+G.borde,borderRadius:7,zIndex:50,maxHeight:220,overflowY:"auto"}}>
                      {hits.map(p=>(<div key={p.id} onClick={()=>agregarItemEv(p)} style={{padding:"9px 14px",cursor:"pointer",display:"flex",justifyContent:"space-between",fontSize:12,borderBottom:"1px solid "+G.borde+"22"}} onMouseEnter={e=>e.currentTarget.style.background=G.sup2} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                        <span>{p.nombre}</span><span style={{color:G.verde,fontFamily:"DM Mono,monospace",fontSize:11}}>{fmt(p.precio_min)}</span>
                      </div>))}
                    </div>);
                  })()}
                </div>
              ...
              <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between",fontSize:13}}>
                <span style={{color:G.textoSec}}>Total{desc>0?" (-"+desc+"%)":""}</span>
                <span style={{fontFamily:"DM Mono,monospace",fontWeight:600,color:G.verde}}>{fmt(total)}</span>
              </div>
            </div>
          </Modal>
        );
      ...
      {confirmarElim&&(
        <Modal title="Eliminar venta" onClose={()=>setConfirmarElim(null)}
          ...
          <p style={{color:G.textoSec,fontSize:13}}>Eliminar la venta de <strong>{confirmarElim.cliente_nombre}</strong> del {confirmarElim.fecha} por <strong>{fmt(confirmarElim.total)}</strong>? Esta accion no se puede deshacer.</p>
        </Modal>
      )}
    </div>
  );
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <Card style={{padding:"12px 18px"}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
          <Fi value={filtroT} onChange={setFT} options={["Todos","Gasto fijo","Gasto variable","Retiro de capital"]}  style={{flex:1,minWidth:150}}/>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:12}}>
        ...
        <Card style={{padding:"12px 18px"}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
            ...
            <Fi value={filtroT} onChange={setFT} options={["Todos","minorista","especial","mayorista","costo"]} style={{width:140}}/>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {quickEditV&&(
        <Modal title="Actualizar venta" onClose={()=>setQuickEditV(null)}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:20}}>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:G.textoSec}}><input type="checkbox" checked={qeCobrado} onChange={e=>setQeCobrado(e.target.checked)}/> Cobrado</label>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:G.textoSec}}><input type="checkbox" checked={qeEntregado} onChange={e=>setQeEntregado(e.target.checked)}/> Entregado</label>
            </div>
            {quickEditV.metodo_pago!=="Efectivo"&&(
              <div style={{background:"#4D9EFF11",border:"1px solid #4D9EFF33",borderRadius:8,padding:"10px 14px"}}>
      ...
      {editandoV&&(()=>{
        ...
        const costos=itemsNum.reduce((s,i)=>s+(i.costo||0)*i.cantidad,0);
        const comision=parseFloat(evComision)||0;
        const ganPreview=totalPreview-costos-comision;
        ...
        <Modal title="Editar venta" onClose={()=>setEditandoV(null)} maxWidth={640}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Fi label="Cliente" value={evCliente} onChange={setEvCliente}
              options={["CONSUMIDOR FINAL",...(clientes||[]).filter(c=>c.activo).map(c=>c.nombre)]}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Vendedor" value={evVendedor} onChange={setEvVendedor} options={(vendedores||[]).map(v=>v.nombre)}/>
            ...
            {conComision&&(
              <div style={{background:"#4D9EFF11",border:"1px solid #4D9EFF33",borderRadius:8,padding:"10px 14px"}}>
                ...
                {comision>0&&<div style={{fontSize:11,color:G.textoSec,marginTop:4}}>Ganancia luego de comision: <strong style={{color:G.verde}}>{fmt(ganPreview)}</strong></div>}
              </div>
            )}
            <div style={{display:"flex",gap:20}}>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:G.textoSec}}><input type="checkbox" checked={evCobrado} onChange={e=>setEvCobrado(e.target.checked)}/> Cobrado</label>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:G.textoSec}}><input type="checkbox" checked={evEntregado} onChange={e=>setEvEntregado(e.target.checked)}/> Entregado</label>
            </div>
            <Div/>
            <ST>Productos de la venta</ST>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 32px",gap:8,fontSize:10,color:G.textoSec,fontWeight:600,textTransform:"uppercase",padding:"0 2px"}}>
                <span>Producto</span><span style={{textAlign:"center"}}>Cant.</span><span style={{textAlign:"right"}}>Precio</span><span/>
              </div>
              {evItems.map((item,idx)=>(
                <div key={idx} style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 32px",gap:8,alignItems:"center"}}>
                  <div style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:7,padding:"7px 10px",fontSize:12,color:G.texto,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.nombre}</div>
                  <input type="number" min="1" value={item.cantidad} onChange={e=>actualizarItem(idx,"cantidad",e.target.value)} style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:7,padding:"7px 8px",color:G.texto,fontSize:12,outline:"none",textAlign:"center",width:"100%"}}/>
                  <input type="number" min="0" value={item.precio} onChange={e=>actualizarItem(idx,"precio",e.target.value)} style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:7,padding:"7px 8px",color:G.texto,fontSize:12,outline:"none",textAlign:"right",width:"100%"}}/>
              ...
              <div style={{marginTop:4,position:"relative"}}>
                <input value={evBusqueda} onChange={e=>setEvBusqueda(e.target.value)} placeholder="+ Buscar y agregar producto..."
                  style={{background:G.sup2,border:`1px solid ${G.verde}55`,borderRadius:7,padding:"8px 12px",color:G.texto,fontSize:12,outline:"none",width:"100%"}}/>
                {evBusqueda.length>1&&(()=>{
                  const hits=(productos||[]).filter(p=>p.nombre.toLowerCase().includes(evBusqueda.toLowerCase())&&p.activo!==false).slice(0,8);
                  ...
                  return(<div style={{position:"absolute",top:"100%",left:0,right:0,background:G.sup,border:`1px solid ${G.borde}`,borderRadius:7,zIndex:50,maxHeight:220,overflowY:"auto"}}>
                    {hits.map(p=>(<div key={p.id} onClick={()=>agregarItemDesdeProducto(p)} style={{padding:"9px 14px",cursor:"pointer",display:"flex",justifyContent:"space-between",fontSize:12,borderBottom:`1px solid ${G.borde}22`}} onMouseEnter={e=>e.currentTarget.style.background=G.sup2} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                      <span>{p.nombre}</span><span style={{color:G.verde,fontFamily:"DM Mono,monospace",fontSize:11}}>{fmt(p.precio_min)}</span>
                    </div>))}
                  </div>);
                })()}
