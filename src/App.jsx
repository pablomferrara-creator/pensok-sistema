...
function useData(toast){
  ...
  return{clientes,productos,ventasConItems,egresos,abastecimiento,vendedores,proveedores,tipoCambio,totalVentas,totalNosDeben,anioStats,traspasos,pagosTraspaso,totalDeudaCamanio,pedidosWeb,pagosEgreso,loading,cargar,cargarPedidosWeb,aceptarPedidoWeb,rechazarPedidoWeb,registrarVenta,registrarEgreso,marcarReembolsado,registrarPagoEgreso,eliminarPagoEgreso,guardarCliente,guardarProducto,registrarAbastecimiento,guardarVendedor,toggleVendedor,guardarProveedor,toggleProveedor,editarVenta,eliminarVenta,editarEgreso,eliminarEgreso,editarAbastecimiento,eliminarAbastecimiento,eliminarProducto,actualizarTipoCambio,actualizarPorcentaje,actualizarDesdeCSV,registrarTraspaso,registrarPagoTraspaso,editarPagoDeuda,eliminarPagoDeuda};
}
...
function ModuloIngresos({ventas,vendedores,productos,clientes,onEditar,onEliminar,onEditarPago,onEliminarPago,totalVentas,filtroInicial="",filtrosPersistentes,onFiltrosChange,esAdmin=true}){
  ...
  async function imprimirRemito(v){
    ...
    const doc = new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
    const W=210,H=297;
    const oscuro=[50,50,50],grisM=[90,90,90],gris=[120,120,120],negro=[30,30,30],blanco=[255,255,255];
    const grisClar=[240,242,245];
    ...
    doc.text('CANT.',W-30,y+5.5,{align:'center'});
    doc.text('RECIBIDO CONFORME',W-16,y+5.5,{align:'right'});
    y+=12;
    let filaPar=false;
    (v.items||[]).forEach(it=>{
      ...
      const nombre=it.nombre.length>55?it.nombre.substring(0,52)+'...':it.nombre;
      doc.text(nombre,16,y);
      doc.setFont('helvetica','bold'); doc.setTextColor(...grisM);
      doc.text(String(it.cantidad||1),W-30,y,{align:'center'});
      // Casilla de firma/confirmación
      doc.setDrawColor(...grisM); doc.setLineWidth(0.2);
      doc.rect(W-28,y-5,18,7);
      y+=8;
    });
    y+=10;
...
function ModuloCaja({ventas,egresos,pagosEgreso=[],toast}){
  const [notas,       setNotas]      = useState("");
  const [loading,     setLoading]    = useState(false);
  ...
  const pagosEgresoDesde = pagosEgreso.filter(p=>p.fecha >= fechaInicio);
  ...
  const calcBolsillo = (bolsillo) => {
    ...
    if(bolsillo==="caja_chica"){
      ...
      return vEf - gEfPagos - gEfLegacy - reEf + movNeto;
    }
    if(bolsillo==="mp"){
      ...
      return vMP_ - gMPPagos - gMPLegacy - reMP + movNeto;
    }
    if(bolsillo==="banco"){
      ...
      return vBa - gBaPagos - gBaLegacy - reBa + movNeto;
    }
    if(bolsillo==="ahorro"){
      return movNeto;
    }
        ...
        <div className="psk-main" style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="caja"           && <ModuloCaja          ventas={data.ventasConItems} egresos={data.egresos} pagosEgreso={data.pagosEgreso} toast={toast}/>}
