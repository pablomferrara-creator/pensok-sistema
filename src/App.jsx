...
function useData(toast){
  ...
  useEffect(()=>{
    cargar();
    // Auto-refresh cada 5 minutos para mantener los datos actualizados
    ...
    for(const cat of cats){
      ...
      for(const p of prods){
        ...
        if(p.stock===0){
          ...
          doc.roundedRect(COL_PRECIO-2,y+1,34,5.5,1,1,'F');
          doc.setFont('helvetica','bold');
          doc.setFontSize(7);
          doc.setTextColor(180,100,0);
          doc.text('CONSULTAR STOCK',COL_PRECIO+15,y+5,{align:'center'});
        } else {
          doc.setFont('helvetica','bold');
          doc.setFontSize(8.5);
          doc.setTextColor(...azul);
              ...
              {filtrados.map((p,i)=>{
                ...
                return(
                  <tr key={p.id} style={{borderBottom:`1px solid ${G.borde}22`,background:i%2===0?"transparent":G.sup2+"44"}}>
                    ...
                    {esAdmin&&<td style={{padding:"9px 12px",display:"flex",gap:6}}><Btn small variant="ghost" onClick={()=>abrirEditar(p)}>Editar</Btn><Btn small variant="danger" onClick={()=>setConfirmarElimProd(p)}>Eliminar</Btn></td>}
                  </tr>
                );
              })}
