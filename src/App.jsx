...
// MODULO: PRODUCTOS
// ============================================================
function ModuloProductos({productos,onGuardar,onEliminar,proveedores,esAdmin=true}){
  ...
  const filtrados=useMemo(()=>{
    ...
    let list=productos.map(p=>({
      ...p,
      gan_min: (p.precio_min||0)-(p.costo||0),
      gan_may: (p.precio_may||0)-(p.costo||0),
    })).filter(p=>{
              ...
              {filtrados.map((p,i)=>{
                ...
                return(
                  <tr key={p.id} style={{borderBottom:`1px solid ${G.borde}22`,background:i%2===0?"transparent":G.sup2+"44"}}>
                    ...
                    {esAdmin&&(()=>{
                      const ganMin=p.precio_min-(p.costo||0);
                      const ganMay=p.precio_may-(p.costo||0);
                      const pctMin=p.costo>0?Math.round((ganMin/p.costo)*100):0;
                      const pctMay=p.costo>0?Math.round((ganMay/p.costo)*100):0;
                      const colMin=pctMin>=60?G.verde:pctMin>=30?G.amarillo:G.rojo;
                      const colMay=pctMay>=30?G.verde:pctMay>=15?G.amarillo:G.rojo;
                      return(<>
                        <td style={{padding:"9px 12px",fontFamily:"DM Mono,monospace",whiteSpace:"nowrap",color:colMin}}>
                          {pF(ganMin)}<span style={{fontSize:9,color:colMin,marginLeft:3}}>({pctMin}%)</span>
                        </td>
                        <td style={{padding:"9px 12px",fontFamily:"DM Mono,monospace",whiteSpace:"nowrap",color:colMay}}>
                          {pF(ganMay)}<span style={{fontSize:9,color:colMay,marginLeft:3}}>({pctMay}%)</span>
                        </td>
                      </>);
                    })()}
        ...
        <div className="psk-main" style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="productos"      && <ModuloProductos      productos={data.productos} onGuardar={data.guardarProducto} onEliminar={data.eliminarProducto} proveedores={data.proveedores} esAdmin={esAdmin}/>}
