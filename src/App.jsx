...
// Cliente secundario para replicar productos nuevos en Caamaño (solo desde Pilar)
const supabaseCamanio = localKey === "pilar"
  ? createClient(LOCALES.camanio.url, LOCALES.camanio.anon)
  : null;
...
function useData(toast){
  ...
  async function guardarProducto(datos,id=null){
    if(id){
      ...
      const CAMPOS_PRODUCTO = ["codigo","nombre","categoria","marca","costo","precio_min","precio_esp","precio_may","stock","stock_min","proveedor","activo","vendidos","moneda"];
      ...
      // Replicar costo en Caamaño si cambió
      if(supabaseCamanio && datos.costo !== undefined){
        try{
          const prod = productos.find(p=>p.id===id);
          if(prod?.codigo){
            await supabaseCamanio.from("productos").update({costo: datos.costo}).eq("codigo", prod.codigo);
          }
        }catch(e){ console.warn("No se pudo replicar costo en Caamaño:", e); }
      }
    } else {
  ...
  async function editarVenta(id,datos,pago=null){
    ...
    // Si viene un pago de deuda, registrarlo en pagos_deuda
    if(pago&&pago.monto>0){
      const venta = ventasConItems.find(v=>v.id===id);
      await supabase.from("pagos_deuda").insert({
        fecha: pago.fecha,
        tipo: "ingreso",
        referencia_id: id,
        concepto: "Cobro deuda - "+(venta?.cliente_nombre||""),
        monto: pago.monto,
        metodo_pago: pago.metodo_pago
      });
    }
  ...
  async function eliminarVenta(id){
    ...
    if(items&&items.length>0){
      for(const item of items){
        ...
        if(prod&&prod.id){
          await supabase.from("productos").update({stock:(prod.stock||0)+(item.cantidad||0)}).eq("id",prod.id);
        }
      }
    }
    await supabase.from("venta_items").delete().eq("venta_id",id);
    await supabase.from("ventas").delete().eq("id",id);
    toast.ok("Venta eliminada y stock repuesto");await cargar();
  }
    ...
    if(supabaseCamanio){
      try{
        ...
        for(const it of items){
          ...
          if(pc) await supabaseCamanio.from("productos").update({stock:(pc.stock||0)+it.cantidad}).eq("id",pc.id);
        }
    ...
    if(supabaseCamanio){
      try{
        await supabaseCamanio.from("egresos").insert({
          ...
          monto, metodo_pago:metodoPago, pagador:"Caamaño",
  ...
  return{clientes,productos,ventasConItems,egresos,abastecimiento,vendedores,proveedores,tipoCambio,totalVentas,totalNosDeben,anioStats,traspasos,pagosTraspaso,totalDeudaCamanio,pedidosWeb,loading,cargar,cargarPedidosWeb,aceptarPedidoWeb,rechazarPedidoWeb,registrarVenta,registrarEgreso,marcarReembolsado,guardarCliente,guardarProducto,registrarAbastecimiento,guardarVendedor,toggleVendedor,guardarProveedor,toggleProveedor,editarVenta,eliminarVenta,editarEgreso,eliminarEgreso,editarAbastecimiento,eliminarAbastecimiento,eliminarProducto,actualizarTipoCambio,actualizarPorcentaje,actualizarDesdeCSV,registrarTraspaso,registrarPagoTraspaso,editarPagoDeuda,eliminarPagoDeuda};
}

// ============================================================
// MODULO: ANALISIS / DASHBOARD
// ============================================================
function ModuloAnalisis({ventas,egresos,productos,vendedores,totalNosDeben,totalDeudaCamanio,anioStats,onNavegar,onFiltroIngresos,onFiltroEgresos}){
...
function ModuloVenta({clientes,productos,onRegistrar,vendedores,esAdmin=true}){
  ...
  async function generarPresupuesto(){
    ...
    doc.setFontSize(8.5);
    doc.text('Tel: 11-7064-5115  ·  @pensok.piletas',46,30);
    // Badge presupuesto
...
// MODULO: INGRESOS
// ============================================================
function ModuloIngresos({ventas,vendedores,productos,clientes,onEditar,onEliminar,onEditarPago,onEliminarPago,totalVentas,filtroInicial="",filtrosPersistentes,onFiltrosChange,esAdmin=true}){
  ...
  async function cargarPagos(){
    ...
    const{data}=await supabase.from("pagos_deuda").select("*").eq("tipo","ingreso").order("created_at",{ascending:false}).limit(300);
    setPagosDeuda(data||[]);
  ...
  async function guardarQuickEdit(){
    ...
    await onEditar(quickEditV.id,{cobrado:qeCobrado,entregado:qeEntregado,comision_plataforma:parseFloat(qeComision)||0});
  ...
  async function imprimirTicket(v){
    ...
    doc.setFont('helvetica','normal');
    doc.setFontSize(8.5);
    doc.text('DOMOKIP SAS',14,23);
    doc.text('Gelves 1126, B1629 Pilar, Buenos Aires',14,28);
    doc.text('CUIT: 30-71686952-7  ·  Tel: 11-7064-5115  ·  @pensok.piletas',14,33);
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(9); doc.setTextColor(...negro);
    doc.text('WhatsApp: 11-7064-5115',16,y+15);
    ...
    doc.text('DOMOKIP SAS  -  CUIT 30-71686952-7  -  Gelves 1126, Pilar, Bs As',14,H-8);
  ...
  async function guardarVenta(){
    ...
    const comision=parseFloat(evComision)||0;
    await onEditar(editandoV.id,{cliente_nombre:evCliente,vendedor:evVendedor,metodo_pago:evMetodo,cobrado:evCobrado,entregado:evEntregado,comision_plataforma:comision,descuento:desc,total,ganancia:total-costos-comision});
    await supabase.from("venta_items").delete().eq("venta_id",editandoV.id);
  ...
  async function guardarQuickEdit(){if(!quickEditV)return;setQeLoading(true);await onEditar(quickEditV.id,{cobrado:qeCobrado,entregado:qeEntregado,comision_plataforma:parseFloat(qeComision)||0});setQeLoading(false);setQuickEditV(null);}
  ...
  const filtrados=useMemo(()=>ventas.filter(v=>{
    ...
    return true;
  }),[ventas,fVend,fMet,fFecha,fEstado,fCliente,fSinComision,fProducto,METODOS_CON_COMISION]);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {tab==="ventas"&&<Card style={{padding:"12px 18px"}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
          ...
          {(fFecha||fEstado||fCliente!=="Todos"||fVend!=="Todos"||fMet!=="Todos"||fSinComision||fProducto)&&<Btn small variant="ghost" onClick={()=>{setFF("");setFEstado("");setFCliente("Todos");setFV("Todos");setFM("Todos");setFSinComision(false);setFProducto("");}}>Limpiar todo</Btn>}
        </div>
      </Card>}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  ...
                  {esAdmin&&!v.cobrado&&<Btn small variant="outline" onClick={e=>{e.stopPropagation();setModalCobro(v);setCobroMonto(String(v.saldo_cobro||v.total||""));setCobroMetodo("Efectivo");setCobroFecha(hoy());}}>💰 Registrar cobro</Btn>}
      ...
      {quickEditV&&(<Modal title="Actualizar venta" onClose={()=>setQuickEditV(null)} footer={<><Btn variant="secondary" onClick={()=>setQuickEditV(null)}>Cancelar</Btn><Btn disabled={qeLoading} onClick={guardarQuickEdit}>{qeLoading?"Guardando...":"Guardar"}</Btn></>}><div style={{display:"flex",flexDirection:"column",gap:14}}><div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.textoSec}}><div style={{fontWeight:600,color:G.texto}}>{quickEditV.cliente_nombre}</div><div>{quickEditV.fecha} · {quickEditV.vendedor} · {fmt(quickEditV.total)}</div></div><div style={{display:"flex",gap:20}}><label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}><input type="checkbox" checked={qeCobrado} onChange={e=>setQeCobrado(e.target.checked)}/> Cobrado</label><label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13}}><input type="checkbox" checked={qeEntregado} onChange={e=>setQeEntregado(e.target.checked)}/> Entregado</label></div>{quickEditV.metodo_pago!=="Efectivo"&&<Fi label="Comision plataforma ($)" value={qeComision} onChange={setQeComision} type="number" placeholder="0"/>}</div></Modal>)}
      {editandoV&&(()=>{
...
function ModuloEgresos({egresos,onRegistrar,onReembolsar,vendedores,proveedores,onEditar,onEliminar,esAdmin=true,filtroInicial="",onConsumirFiltro}){
  ...
  async function guardarEgreso(){
    ...
    await onEditar(editandoEg.id,{concepto:efConcepto,monto:montoT,tipo:efTipo,metodo_pago:efMetodo,pagador:efPagador,notas:efNotas,monto_reembolsado:esInvEdit?montoT:montoR,saldo_pendiente:esInvEdit?0:saldoPend,reembolso_pendiente:esInvEdit?false:(efPagador!=="Pensok"&&saldoPend>0),reembolsado:esInvEdit?true:(efPagador!=="Pensok"&&saldoPend===0)});
  ...
  const [fAbonado,setFAbonado]=useState("total");
  const [fAbonMonto,setFAbonMonto]=useState("");
  const [loading,setLoading]=useState(false);
  ...
  const filtrados=useMemo(()=>egresos.filter(e=>{if(filtroReemb&&!(e.reembolso_pendiente&&!e.reembolsado))return false;if(filtroT!=="Todos"&&e.tipo!==filtroT)return false;if(filtroP!=="Todos"&&e.pagador!==filtroP)return false;if(filtroF&&e.fecha!==filtroF)return false;return true;}),[egresos,filtroReemb,filtroT,filtroP,filtroF]);
  ...
  async function guardar(){
    ...
    const montoAbonado=fAbonado==="parcial"?(parseFloat(fAbonMonto)||0):montoT;
    const saldo=montoT-montoAbonado;
    const deudaProv=fPagador==="Pensok"&&saldo>0; // Pensok le debe al proveedor
    const esInversion = fTipo==="Inversión inicial";
    await onRegistrar({
      fecha:fFecha,concepto:fConcepto,tipo:fTipo,monto:montoT,metodo_pago:fMetodo,
      pagador:fPagador,
      reembolso_pendiente:esInversion?false:(reembolso||deudaProv),
      reembolsado:esInversion?true:false,
      monto_reembolsado:esInversion?montoT:(montoAbonado<montoT?montoAbonado:0),
      saldo_pendiente:esInversion?0:(saldo>0?saldo:0),
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <Card key={e.id} style={{padding:"12px 18px",border:e.reembolso_pendiente&&!e.reembolsado?`1px solid #FFB80033`:undefined}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                ...
                {esAdmin&&e.reembolso_pendiente&&!e.reembolsado&&<Btn small variant="outline" onClick={()=>{setModalReemb(e);setReembMonto(String(e.saldo_pendiente||e.monto||''));setReembMetodo('Efectivo');setReembFecha(hoy());}}>Registrar pago</Btn>}
                <div style={{display:"flex",gap:6}}>
      ...
      {modalReemb&&(
        <Modal title="Registrar pago" onClose={()=>setModalReemb(null)}
          footer={<>
            <Btn variant="secondary" onClick={()=>setModalReemb(null)}>Cancelar</Btn>
            <Btn disabled={!reembMonto||parseFloat(reembMonto)<=0} onClick={async()=>{
              await onReembolsar(modalReemb.id,{monto:parseFloat(reembMonto),metodo_pago:reembMetodo,fecha:reembFecha,concepto:modalReemb.concepto});
              setModalReemb(null);
            }}>Registrar</Btn>
          </>}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.textoSec}}>
              <div style={{fontWeight:600,color:G.texto}}>{modalReemb.concepto}</div>
              <div>Pagador: <strong>{modalReemb.pagador}</strong> · Total: <strong style={{color:G.rojo}}>{fmt(modalReemb.monto)}</strong></div>
              {(modalReemb.monto_reembolsado||0)>0&&<div>Ya abonado: <strong style={{color:G.verde}}>{fmt(modalReemb.monto_reembolsado)}</strong> · Saldo: <strong style={{color:G.amarillo}}>{fmt(modalReemb.saldo_pendiente||0)}</strong></div>}
            </div>
            <Fi label="Monto a pagar ahora ($)" value={reembMonto} onChange={setReembMonto} type="number" placeholder="0"/>
            <Fi label="Método de pago" value={reembMetodo} onChange={setReembMetodo} options={METODOS_PAGO}/>
            <Fi label="Fecha del pago" value={reembFecha} onChange={setReembFecha} type="date"/>
          </div>
        </Modal>
      )}
      {confirmarElimEg&&(
      ...
      {modal&&(
        <Modal title="Registrar egreso" onClose={()=>setModal(false)}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {/* Pago parcial a proveedor — solo si pagador es Pensok */}
            {!reembolso&&fMonto&&(
              <div style={{border:"1px solid "+G.borde,borderRadius:8,padding:"12px 14px",display:"flex",flexDirection:"column",gap:10}}>
                <div style={{fontSize:11,color:G.textoSec,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5}}>Pago a proveedor</div>
                <div style={{display:"flex",gap:10}}>
                  {[{k:"total",l:"Abonado total"},{k:"parcial",l:"Pago parcial"}].map(o=>(
                    <button key={o.k} onClick={()=>setFAbonado(o.k)}
                      style={{flex:1,padding:"9px",borderRadius:8,border:"2px solid "+(fAbonado===o.k?G.verde:G.borde),background:fAbonado===o.k?"#00C48C18":G.sup2,color:fAbonado===o.k?G.verde:G.textoSec,fontWeight:600,fontSize:12,cursor:"pointer"}}>
                      {o.l}
                    </button>
                  ))}
                </div>
                {fAbonado==="parcial"&&(
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    <Fi label="Monto abonado hasta ahora ($)" value={fAbonMonto} onChange={setFAbonMonto} type="number" placeholder="0"/>
                    {fAbonMonto&&parseFloat(fAbonMonto)<parseFloat(fMonto)&&(
                      <div style={{background:"#FFB80011",border:"1px solid #FFB80033",borderRadius:8,padding:"8px 12px",fontSize:12,color:G.amarillo}}>
                        Saldo pendiente con proveedor: <strong>{fmt(parseFloat(fMonto)-(parseFloat(fAbonMonto)||0))}</strong> — quedará como reembolso pendiente.
                      </div>
                    )}
                  </div>
                )}
...
function ModuloClientes({clientes,onGuardar,ventas}){
  ...
  async function generarDeudaPDF(){
    ...
    doc.setFont('helvetica','normal');
    doc.setFontSize(8.5);
    doc.text('Tel: 11-7064-5115  ·  @pensok.piletas',44,23);
...
// MODULO: PRODUCTOS
// ============================================================
function ModuloProductos({productos,onGuardar,onEliminar,proveedores,ventas=[],esAdmin=true}){
  ...
  async function generarListaPDF(tipo, productos_filtrados, filtrosInfo={}){
    ...
    function dibujarEncabezado(){
      ...
      // Contacto
      doc.setFontSize(8.5);
      doc.text('Tel: 11-7064-5115',44,29);
    ...
    dibujarEncabezado();
    let y = 44;
    const COL_PROD=14, COL_CAT=148, COL_PRECIO=174;
    ...
    for(const cat of cats){
      ...
      for(const p of prods){
        ...
        doc.setTextColor(...gris);
        doc.setFontSize(7.5);
        doc.text(p.marca||'',COL_CAT,y+5);
        // Precio / stock — siempre se muestra el precio; si no hay stock, además se agrega el cartel
        ...
        if(p.stock===0){
          ...
          doc.roundedRect(COL_PRECIO-2,y+ROW_H-1,34,4.5,1,1,'F');
          doc.setFont('helvetica','bold');
          doc.setFontSize(6);
          doc.setTextColor(180,100,0);
          doc.text('CONSULTAR STOCK',COL_PRECIO+15,y+ROW_H+2.2,{align:'center'});
        }
              ...
              {filtrados.map((p,i)=>{
                ...
                return(
                  <tr key={p.id} style={{borderBottom:`1px solid ${G.borde}22`,background:i%2===0?"transparent":G.sup2+"44"}}>
                    ...
                    {esAdmin&&<td style={{padding:"9px 12px",display:"flex",gap:6}}><Btn small variant="ghost" onClick={()=>abrirEditar(p)}>Editar</Btn><Btn small variant={p.activo?"secondary":"outline"} onClick={()=>onGuardar({activo:!p.activo},p.id)}>{p.activo?"Desactivar":"Activar"}</Btn><Btn small variant="danger" onClick={()=>setConfirmarElimProd(p)}>Eliminar</Btn></td>}
                  </tr>
                );
              })}
...
function ModuloTraspasos({traspasos,pagosTraspaso,productos,onRegistrar,onPago,totalDeudaCamanio,localKey,toast}){
  ...
  const [modalPago,    setModalPago]    = useState(null); // traspaso seleccionado
  const [detalleTraspaso, setDetalleTraspaso] = useState(null); // traspaso para ver detalle completo
  const [busqueda,     setBusqueda]     = useState("");
  ...
  return(<>
    ...
    {detalleTraspaso&&(
      <Modal title={`Traspaso del ${detalleTraspaso.fecha}`} onClose={()=>setDetalleTraspaso(null)} maxWidth={560}
        footer={<Btn variant="secondary" onClick={()=>setDetalleTraspaso(null)}>Cerrar</Btn>}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
...
function ModuloAbastecimiento({productos,abastecimiento,onRegistrar,vendedores,proveedores,onEditar,onEliminar}){
  ...
  async function registrar(){
    ...
    await onRegistrar({fecha:hoy(),producto_id:prodSelec.id,nombre:prodSelec.nombre,cantidad:parseInt(cantidad),costo_unit:parseFloat(costoUnit)||prodSelec.costo||0,proveedor,metodo_pago:metodo||METODOS_PAGO[0],responsable:resp||"Pensok",notas});
    setLoading(false);setOk(true);
...
function ModuloCaja({ventas,egresos,toast}){
  ...
  // Snapshot instantáneo: fecha y hora actuales al momento de abrir/actualizar
  const ahora = new Date();
  const fechaAhora = ahora.toISOString().slice(0,10); // "YYYY-MM-DD"
  const tsAhora    = ahora.toISOString();              // timestamp completo

  useEffect(()=>{ cargarHistorial(); },[]);

  async function cargarHistorial(){
    const[{data:c},{data:m},{data:pd},{data:pt}]=await Promise.all([
      ...
      supabase.from("pagos_deuda").select("*").order("fecha",{ascending:false}).limit(500),
      localKey==="pilar" ? supabase.from("pagos_traspaso").select("*").order("fecha",{ascending:false}).limit(200) : {data:[]},
    ]);
    setCierres(c||[]);
    setMovimientos(m||[]);
  ...
  // Último cierre guardado (por created_at)
  const cierreAnterior = useMemo(()=>cierres[0]||null,[cierres]);
  const esPrimerCierre = cierres.length===0;

  // Saldo inicial: del último cierre guardado, o lo que ingresa el usuario si es el primer cierre
  ...
  // Timestamp exacto del último cierre (para corte preciso de ventas)
  const tsUltimoCierre = cierreAnterior?.created_at || null;
  // Fecha del último cierre (para egresos, que no tienen hora)
  const fechaUltimoCierre = tsUltimoCierre ? tsUltimoCierre.slice(0,10) : null;

  // Días transcurridos desde el último cierre
  const diasSinCierre = fechaUltimoCierre
    ? (()=>{
        const d1=new Date(fechaUltimoCierre+"T00:00:00");
  ...
  const MP_METODOS_CD   = ["Transferencia MP","Debito MP","Credito MP"];
  const BANCO_METODOS_CD= ["Transferencia Banco","Debito Banco","Credito Banco","Credito Cuotas Banco"];
  // Tipos de egreso que NO afectan caja (inversiones no son movimientos de billetera)
  const TIPOS_EGRESO_EXCLUIDOS = ["Inversión inicial"];

  // ── Ventas desde el último cierre hasta ahora (corte por hora exacta) ──
  const ventasDesde = tsUltimoCierre
    ? ventas.filter(v=>{
        if(!v.created_at) return v.fecha > fechaUltimoCierre;
        return v.created_at > tsUltimoCierre;
      })
    : ventas.filter(v=>v.fecha >= fechaAhora);

  // ── Egresos desde el día del último cierre inclusive (no tienen hora) ──
  const egresosDesde = fechaUltimoCierre
    ? egresos.filter(e=>e.fecha >= fechaUltimoCierre && !TIPOS_EGRESO_EXCLUIDOS.includes(e.tipo))
    : egresos.filter(e=>e.fecha >= fechaAhora && !TIPOS_EGRESO_EXCLUIDOS.includes(e.tipo));

  // ── Pagos de deuda desde el último cierre ──
  const pagosDesde = tsUltimoCierre
    ? pagosDia.filter(p=>{
        if(!p.created_at) return p.fecha > fechaUltimoCierre;
        return p.created_at > tsUltimoCierre;
      })
    : pagosDia.filter(p=>p.fecha >= fechaAhora);

  // ── Movimientos de caja desde el último cierre ──
  const movsDesde = fechaUltimoCierre
    ? movimientos.filter(m=>m.fecha >= fechaUltimoCierre)
    : movimientos.filter(m=>m.fecha >= fechaAhora);

  // ── Cálculo por bolsillo del período completo desde el último cierre ──
  const calcBolsillo = (bolsillo) => {
    ...
    if(bolsillo==="caja_chica"){
      ...
      const vEf   = ventasDesde.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+(v.total||0),0)+cobEf;
      const gEf   = egresosDesde.filter(e=>e.metodo_pago==="Efectivo").reduce((s,e)=>s+(e.monto||0),0)+reEf;
      return vEf - gEf + movNeto;
    }
    if(bolsillo==="mp"){
      const cobMP = cobros.filter(p=>MP_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
      const vMP_  = ventasDesde.filter(v=>MP_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0)+cobMP;
      return vMP_ + movNeto;
    }
    if(bolsillo==="banco"){
      const cobBa = cobros.filter(p=>BANCO_METODOS_CD.includes(p.metodo_pago)).reduce((s,p)=>s+p.monto,0);
      const vBa   = ventasDesde.filter(v=>BANCO_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0)+cobBa;
      return vBa + movNeto;
    }
    return 0;
  };

  // Esperado = saldo inicial + movimientos del período
  const esperadoCaja   = inicialCaja   + calcBolsillo("caja_chica");
  const esperadoMP     = inicialMP     + calcBolsillo("mp");
  const esperadoBanco  = inicialBanco  + calcBolsillo("banco");

  // Totales del período (para mostrar en la card de resumen)
  const vEfectivo = ventasDesde.filter(v=>v.metodo_pago==="Efectivo").reduce((s,v)=>s+(v.total||0),0);
  const vMP       = ventasDesde.filter(v=>MP_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0);
  const vBanco    = ventasDesde.filter(v=>BANCO_METODOS.includes(v.metodo_pago)).reduce((s,v)=>s+(v.total||0),0);
  const vCC       = ventasDesde.filter(v=>v.metodo_pago==="Cuenta corriente").reduce((s,v)=>s+(v.total||0),0);
  const vTotal    = ventasDesde.reduce((s,v)=>s+(v.total||0),0);
  const vTransf   = vBanco;
  const vDebito   = 0;
  const vCredito  = 0;
  const gEfectivo = egresosDesde.filter(e=>e.metodo_pago==="Efectivo").reduce((s,e)=>s+(e.monto||0),0);
  const gOtros    = egresosDesde.filter(e=>e.metodo_pago!=="Efectivo").reduce((s,e)=>s+(e.monto||0),0);
  const gTotal    = egresosDesde.reduce((s,e)=>s+(e.monto||0),0);
  const ganNeta   = ventasDesde.reduce((s,v)=>s+(v.ganancia||0),0);
  ...
  // Fecha a guardar: fecha actual (el cierre es siempre "ahora")
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {tab==="nuevo"&&(<>
        ...
        <Card style={{background:G.sup2,border:`1px solid ${G.borde}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
            <div>
              <div style={{fontSize:13,fontWeight:600}}>Snapshot actual — {fechaAhora}</div>
              <div style={{fontSize:11,color:G.textoSec,marginTop:2}}>
                {tsUltimoCierre
                  ? `Acumulado desde cierre del ${fechaUltimoCierre} · ${diasSinCierre} día${diasSinCierre!==1?"s":""} · ${ventasDesde.length} ventas · ${egresosDesde.length} egresos`
                  : `Primer cierre · ${ventasDesde.length} ventas · ${egresosDesde.length} egresos en sistema`}
              </div>
            </div>
            <Btn small variant="secondary" onClick={cargarHistorial}>↺ Actualizar</Btn>
          </div>
        </Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <Card>
              <ST>Ingresos del período — sistema</ST>
              {[
            ...
            <Card>
              <ST>Egresos del período — sistema</ST>
              {[
            ...
            <Card style={{border:`1px solid ${G.verde}33`}}>
              <ST>Ganancia neta del período</ST>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {esPrimerCierre ? (
              <Card style={{border:`1px solid ${G.amarillo}44`,background:"#FFB80008"}}>
                <ST>⚡ Primer cierre — saldos de arranque</ST>
                <div style={{fontSize:12,color:G.textoSec,marginBottom:10}}>
                  Ingresá cuánto hay en cada bolsillo ahora mismo. A partir del próximo cierre el sistema lo toma automáticamente.
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  <Fi label="Caja Chica" value={initCaja} onChange={setInitCaja} type="number" placeholder="0"/>
                  <Fi label="Mercado Pago" value={initMP} onChange={setInitMP} type="number" placeholder="0"/>
                  <Fi label="Banco" value={initBanco} onChange={setInitBanco} type="number" placeholder="0"/>
                  <Fi label="Ahorro" value={initAhorro} onChange={setInitAhorro} type="number" placeholder="0"/>
                </div>
              </Card>
            ) : cierreAnterior ? (
              <Card style={{background:G.sup2,border:`1px solid ${G.borde}`}}>
                <ST>Base — cierre del {cierreAnterior.fecha}{diasSinCierre>0?` · ${diasSinCierre} día${diasSinCierre!==1?"s":""} acumulado${diasSinCierre!==1?"s":""}`:""}</ST>
            ...
            {(cajChica||saldoMP||saldoBanco)&&(
              <Card>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  ...
                  {diasSinCierre>0&&(
                    <span style={{fontSize:11,color:G.amarillo,background:"#FFB80015",border:"1px solid #FFB80033",borderRadius:6,padding:"2px 8px"}}>
                      {diasSinCierre} día{diasSinCierre!==1?"s":""} acumulado{diasSinCierre!==1?"s":""}
                    </span>
                  )}
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {[
                    {l:"Caja Chica",  esp:esperadoCaja,  real:realCaja,  diffAcum:diffCaja,  diffDia:diffCaja,
                      det:`Base ${fmt(inicialCaja)} + ventas ef. ${fmt(vEfectivo)} − gastos ef. ${fmt(gEfectivo)}`},
                    {l:"Mercado Pago",esp:esperadoMP,    real:realMP,    diffAcum:diffMP,    diffDia:diffMP,
                      det:`Base ${fmt(inicialMP)} + cobros MP ${fmt(vMP)}`},
                    {l:"Banco",       esp:esperadoBanco, real:realBanco, diffAcum:diffBanco, diffDia:diffBanco,
                      det:`Base ${fmt(inicialBanco)} + cobros banco ${fmt(vBanco)}`},
                  ].map(x=>(
                    <div key={x.l} style={{background:G.sup2,borderRadius:8,padding:"10px 14px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                        <span style={{fontSize:12,fontWeight:600}}>{x.l}</span>
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <span style={{fontSize:13,fontWeight:700,color:diffColor(x.diffAcum)}}>
                            {diffLabel(x.diffAcum)}
                          </span>
                        </div>
                      </div>
                      ...
                      <div style={{display:"flex",gap:16,fontSize:11,color:G.textoSec}}>
                        ...
                        <span>Real contado: <strong style={{color:G.texto}}>{fmt(x.real)}</strong></span>
                      </div>
                    </div>
                  ))}
                  {/* Total diferencia acumulada */}
                  {(()=>{
                    const totalDiff = diffCaja + diffMP + diffBanco;
                    return(
                      <div style={{borderTop:`1px solid ${G.borde}`,paddingTop:10,marginTop:2,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span style={{fontSize:13,fontWeight:600}}>Diferencia total acumulada</span>
                        <span style={{fontSize:16,fontWeight:700,fontFamily:"DM Mono,monospace",color:diffColor(totalDiff)}}>
                          {totalDiff===0?"✓ Cuadra":totalDiff>0?`+${fmt(totalDiff)} sobrante`:`${fmt(totalDiff)} faltante`}
                        </span>
                      </div>
                    );
                  })()}
        ...
        <div className="psk-main" style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="ingresos"       && <ModuloIngresos       ventas={data.ventasConItems} vendedores={data.vendedores} productos={data.productos} clientes={data.clientes} onEditar={data.editarVenta} onEliminar={data.eliminarVenta} onEditarPago={data.editarPagoDeuda} onEliminarPago={data.eliminarPagoDeuda} totalVentas={data.totalVentas} filtroInicial={filtroIngresos} filtrosPersistentes={ingFiltros} onFiltrosChange={setIngFiltros} esAdmin={esAdmin}/>}
              ...
              {modulo==="egresos"        && <ModuloEgresos  esAdmin={esAdmin}        egresos={data.egresos} onRegistrar={data.registrarEgreso} onReembolsar={data.marcarReembolsado} vendedores={data.vendedores} proveedores={data.proveedores} onEditar={data.editarEgreso} onEliminar={data.eliminarEgreso} filtroInicial={filtroEgresos} onConsumirFiltro={()=>setFiltroEgresos("")}/>}
              {modulo==="clientes"       && <ModuloClientes       clientes={data.clientes} onGuardar={data.guardarCliente} ventas={data.ventasConItems}/>}
              {modulo==="productos"      && <ModuloProductos      productos={data.productos} onGuardar={data.guardarProducto} onEliminar={data.eliminarProducto} proveedores={data.proveedores} ventas={data.ventasConItems} esAdmin={esAdmin}/>}
