  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {tab==="libro"&&(()=>{
        ...
        // 1. Ventas cobradas (usando pagos_deuda con referencia_id para tener fecha real)
        ...
        // 2. Ventas cobradas sin registro en pagos_deuda (implícito)
        const idsConPago = new Set(pagosVentas.map(p=>p.referencia_id));
        ventas.filter(v=>v.cobrado&&!idsConPago.has(v.id)&&v.fecha>=fechaInicio).forEach(v=>{
          const billetera = MP_METODOS.includes(v.metodo_pago)?"mp":BANCO_METODOS.includes(v.metodo_pago)?"banco":"caja_chica";
          const monto = (v.total||0) - (v.metodo_pago!=="Efectivo"?v.comision_plataforma||0:0);
          movs.push({
        ...
        pagosDia.filter(p=>p.tipo==="ingreso"&&!p.referencia_id&&p.fecha>=fechaInicio).forEach(p=>{
          const billetera = MP_METODOS.includes(p.metodo_pago)?"mp":BANCO_METODOS.includes(p.metodo_pago)?"banco":"caja_chica";
          movs.push({
        ...
        pagosDia.filter(p=>p.traspaso_id&&p.fecha>=fechaInicio).forEach(p=>{
          const billetera = MP_METODOS.includes(p.metodo_pago)?"mp":BANCO_METODOS.includes(p.metodo_pago)?"banco":"caja_chica";
          movs.push({
        ...
        // 5. Pagos de egresos (pagos_egreso con fecha real)
        pagosEgreso.filter(p=>p.fecha>=fechaInicio).forEach(p=>{
        ...
        // 6. Egresos legacy (sin pagos_egreso, pagados por Pensok)
        const egresosConPagos2 = new Set(pagosEgreso.map(p=>p.egreso_id));
        egresosDesde.filter(e=>!egresosConPagos2.has(e.id)).forEach(e=>{
          ...
          movs.push({
            ...
            concepto:e.concepto,
            metodo:e.metodo_pago, monto:e.monto, billetera,
            origen:"Gasto/Egreso",
          });
        });
