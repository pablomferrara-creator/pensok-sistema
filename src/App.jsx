...
function PanelReposicion({alertas}){
  ...
  const alertasFiltradas = useMemo(()=>{
    if(filtroProv==="Todos") return [...alertas].sort((a,b)=>(a.proveedor||"").localeCompare(b.proveedor||""));
    return alertas.filter(p=>(p.proveedor||"Sin proveedor")===filtroProv);
  },[alertas,filtroProv]);
  ...
  function exportarCSV(){
    ...
    const lines=[["Codigo","Producto","Proveedor","Stock actual","Stock minimo","Cantidad a pedir"]];
    seleccionados.forEach(p=>{
      lines.push([p.codigo,p.nombre,p.proveedor||"",p.stock,p.stock_min,cantidades[p.id]||Math.max(0,p.stock_min*2-p.stock)]);
    });
    const csv=lines.map(r=>r.join(",")).join("\n");
    const blob=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url;a.download=`pedido-reposicion-${new Date().toISOString().split("T")[0]}.csv`;
        ...
        <div style={{background:G.sup,border:"1px solid #FF4D6A33",borderTop:"none",borderRadius:"0 0 12px 12px",padding:"16px 18px"}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:14}}>
            ...
            {seleccionados.length>0&&<>
              ...
              <Btn small variant="outline" onClick={exportarCSV}>↓ Exportar CSV</Btn>
