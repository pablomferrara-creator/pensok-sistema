...
function ModuloProductos({productos,onGuardar,onEliminar,proveedores,ventas=[],esAdmin=true}){
  ...
  function calcularRecompra(){
    ...
    // Ventas por producto en esos meses (con items embebidos)
    const ventasPorProd = {};
    ventas.forEach(v=>{
      ...
      (v.items||[]).forEach(it=>{
        ...
        if(!ventasPorProd[it.producto_id]) ventasPorProd[it.producto_id]={cant:0,gan:0,mesesConVenta:new Set()};
        ventasPorProd[it.producto_id].cant+=(it.cantidad||0);
        ventasPorProd[it.producto_id].gan+=(it.ganancia||(it.precio-(it.costo||0))*it.cantidad)||0;
        ventasPorProd[it.producto_id].mesesConVenta.add(mes);
      });
    });

    // Calcular score y cantidad proyectada por producto
    const lineas = [];
    const totalMeses = todosMeses.length; // 2 × mesesHist

    productos.filter(p=>p.activo&&p.costo>0).forEach(p=>{
      const hist = ventasPorProd[p.id];
  ...
  return(
    ...
    {modalRecompra&&(
      <Modal onClose={()=>setModalRecompra(false)} title="🛒 Reporte de Compra Inteligente">
        {!rcResultado?(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14,minWidth:Math.min(700,window.innerWidth-40),maxWidth:"90vw"}}>
...
function ModuloCaja({ventas,egresos,toast}){
  ...
  async function cargarHistorial(){
    const[{data:c},{data:m},{data:pd}]=await Promise.all([
