...
function useData(toast){
  ...
  // ── ACTUALIZAR PRECIOS ───────────────────────────────────
  async function actualizarTipoCambio(nuevoTC){
    const provsUSD=(proveedores||[]).filter(p=>p.moneda==="USD");
    const nombresUSD=provsUSD.map(p=>p.nombre);
    const prodsUSD=productos.filter(p=>nombresUSD.includes(p.proveedor));
    for(const prod of prodsUSD){
      const prov=provsUSD.find(p=>p.nombre===prod.proveedor);
      const desc=(prov?.descuento||0)/100;
      const costoBase=prod.costo; // costo original en USD
      const costoARS=Math.round(costoBase*nuevoTC*(1-desc));
      const ganMin=(prod.ganancia_min||0); const ganMay=(prod.ganancia_may||0);
      await supabase.from("productos").update({
        precio_min:Math.round(costoARS*(1+ganMin/100)),
        precio_esp:Math.round(costoARS*(1+ganMin/100)*0.95),
        precio_may:Math.round(costoARS*(1+ganMay/100))
      }).eq("id",prod.id);
    }
    setTipoCambio(nuevoTC);
    toast.ok(`TC $${nuevoTC.toLocaleString("es-AR")} — ${prodsUSD.length} productos recalculados`);
    await cargar();
  }
...
function ModuloActualizarPrecios({proveedores,productos,tipoCambio,onActualizarTC,onActualizarPct,onActualizarCSV}){
  ...
  async function ejecutarTC(){
    const tc=parseFloat(nuevoTC);
    if(!tc||tc<1)return;
    setLoading(true);
    await onActualizarTC(tc);
      ...
      {modo==="tc"&&(
        <Card>
          ...
          <div style={{fontSize:13,color:G.textoSec,marginBottom:14}}>
            Al confirmar, se recalculan los precios de venta de todos los productos de proveedores en USD,
            aplicando automáticamente el descuento de cada proveedor y los márgenes de cada producto.
          </div>
          <div style={{display:"flex",gap:12,alignItems:"flex-end",flexWrap:"wrap"}}>
            ...
            <div style={{fontSize:13,color:G.textoSec}}>
              Actual: <strong>${Number(tipoCambio).toLocaleString("es-AR")}</strong>
              {nuevoTC&&parseFloat(nuevoTC)!==tipoCambio&&<span style={{color:parseFloat(nuevoTC)>tipoCambio?G.rojo:G.verde,marginLeft:8}}>
                {parseFloat(nuevoTC)>tipoCambio?'▲':'▼'} {Math.abs(Math.round((parseFloat(nuevoTC)/tipoCambio-1)*100))}%
              </span>}
            </div>
          </div>
          {provsUSD.length>0&&(
            <div style={{marginTop:14,background:G.sup2,borderRadius:10,padding:"12px 14px"}}>
              <div style={{fontSize:12,color:G.textoSec,marginBottom:8}}>Proveedores USD afectados:</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {provsUSD.map(p=>(
                  <div key={p.id} style={{fontSize:12,background:G.fondo,borderRadius:8,padding:"4px 10px"}}>
                    {p.nombre} {p.descuento>0&&<span style={{color:G.verde}}>(-{p.descuento}%)</span>}
                    <span style={{color:G.textoSec,marginLeft:4}}>
                      ({productos.filter(pr=>pr.proveedor===p.nombre).length} productos)
                    </span>
                  </div>
                ))}
              </div>
          ...
          {!confirmar
            ?<Btn style={{marginTop:14}} disabled={!nuevoTC||parseFloat(nuevoTC)===tipoCambio} onClick={()=>setConfirmar(true)}>
            ...
            :<div style={{marginTop:14,background:"#FFB80011",border:"1px solid #FFB80033",borderRadius:10,padding:"14px 16px"}}>
               <div style={{fontSize:13,color:G.amarillo,fontWeight:600,marginBottom:10}}>
                 ⚠ Se actualizarán los precios de {productos.filter(p=>provsUSD.map(v=>v.nombre).includes(p.proveedor)).length} productos
               </div>
               <div style={{display:"flex",gap:10}}>
                 <LoadBtn onClick={ejecutarTC}>Confirmar actualización</LoadBtn>
