  ...
  async function generarListaPDF(tipo, productos_filtrados){
    ...
    for(const cat of cats){
      ...
      for(const p of prods){
        ...
        // Precio / stock
        const precio = tipo==='minorista' ? p.precio_min : tipo==='especial' ? p.precio_esp : p.precio_may;
        if(p.stock===0){
      ...
      {modalLista&&(
        <Modal title="Exportar lista de precios" onClose={()=>setModalLista(false)}
          footer={<><Btn variant="secondary" onClick={()=>setModalLista(false)}>Cancelar</Btn><Btn disabled={generando} onClick={()=>{
              ...
              generarListaPDF(tipoLista,base);
