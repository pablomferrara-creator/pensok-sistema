...
function useData(toast){
  ...
  async function guardarProducto(datos,id=null){
    if(id){
      // Limpiar campos que pueden causar conflicto
      const datosLimpios = Object.fromEntries(Object.entries(datos).filter(([k,v])=>v!==undefined&&v!==null&&k!=="id"&&k!=="created_at"&&k!=="granel_id"&&k!=="consumo_granel"));
      console.log("UPDATE producto datos:", JSON.stringify(datosLimpios));
      const{error, status}=await supabase.from("productos").update(datosLimpios).eq("id",id);
      if(error){console.error("Supabase error:", status, error); toast.err("Error al actualizar producto: "+error.message);return;}
...
function ModuloVenta({clientes,productos,onRegistrar,vendedores,esAdmin=true}){
  ...
  function cambiarCliente(nuevoId){
    ...
    if(tipo==="mayorista"||modalidad==="Telefonica / Delivery"){
      setDescuento("0");
    } else {
      setDescuento(String(DESC_POR_METODO[metodo]??0));
    }
