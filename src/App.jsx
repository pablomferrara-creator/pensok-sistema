...
const supabaseCamanio = localKey === "pilar"
  ...
  : null;
// Cliente bidireccional "al otro local" (Pilar↔Caamaño) — usado para proveedores,
// que a diferencia de productos se sincronizan en los dos sentidos
const otroLocalKey = localKey === "pilar" ? "camanio" : "pilar";
const supabaseOtro = createClient(LOCALES[otroLocalKey].url, LOCALES[otroLocalKey].anon);
...
function useData(toast){
  ...
  // Cambia el estado. Al marcarla hecha se guarda quién y cuándo: eso alimenta el historial del calendario.
  async function cambiarEstadoTarea(id,estado,quien){
    const datos = estado==="hecha"
      ? {estado, completada_at:new Date().toISOString(), completada_por:quien||""}
      : {estado, completada_at:null, completada_por:null};
    const{error}=await supabaseTareas.from("tareas").update(datos).eq("id",id);
...
function ModuloTareas({tareas=[],responsables=[],onGuardar,onCambiarEstado,onEliminar,esAdmin=true,usuarioEmail=""}){
  ...
  async function guardar(){
    ...
    if(fEstado==="hecha"){
      ...
      datos.completada_at=null; datos.completada_por=null;
    }
  ...
  const Fila = ({t})=>{
    ...
    return(
      <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 12px",background:G.sup2,border:`1px solid ${vencida?G.rojo+"55":G.borde}`,borderRadius:9,borderLeft:`3px solid ${hecha?G.verde:colorPrioridad(t.prioridad)}`}}>
        <input type="checkbox" checked={hecha} style={{marginTop:3,cursor:"pointer",flexShrink:0}}
          onChange={e=>onCambiarEstado(t.id, e.target.checked?"hecha":"pendiente", usuarioEmail)}/>
        ...
        <div style={{display:"flex",gap:4,flexShrink:0}}>
          {!hecha&&t.estado!=="en_curso"&&<Btn small variant="ghost" onClick={()=>onCambiarEstado(t.id,"en_curso",usuarioEmail)}>▶ En curso</Btn>}
          <Btn small variant="ghost" onClick={()=>abrirEditar(t)}>Editar</Btn>
        ...
        <div className="psk-main" style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="tareas"         && <ModuloTareas         tareas={data.tareas} responsables={data.responsables} onGuardar={data.guardarTarea} onCambiarEstado={data.cambiarEstadoTarea} onEliminar={data.eliminarTarea} esAdmin={esAdmin} usuarioEmail={session?.user?.email||""}/>}
