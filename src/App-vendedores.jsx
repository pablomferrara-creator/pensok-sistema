...
function useData(toast){
  ...
  async function cargar(){
    setLoading(true);
    const[{data:cls},{data:prds},{data:vts},{data:vis},{data:egs},{data:abs},{data:vends}]=await Promise.all([
    ...
    setClientes(cls||[]);setProductos(prds||[]);setVentas(vts||[]);setVentaItems(vis||[]);setEgresos(egs||[]);setAbastecimiento(abs||[]);setVendedores(vends||[]);
    setLoading(false);
  }
  ...
  return{clientes,productos,ventasConItems,egresos,abastecimiento,vendedores,loading,cargar,registrarVenta,registrarEgreso,marcarReembolsado,guardarCliente,guardarProducto,registrarAbastecimiento,guardarVendedor,toggleVendedor};
}
...
// MODULO: INGRESOS
// ============================================================
function ModuloIngresos({ventas,vendedores}){
...
// MODULO: EGRESOS
// ============================================================
function ModuloEgresos({egresos,onRegistrar,onReembolsar,vendedores}){
...
// ============================================================
function ModuloAbastecimiento({productos,abastecimiento,onRegistrar,vendedores}){
        ...
        <div style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              ...
              {modulo==="ingresos"       && <ModuloIngresos       ventas={data.ventasConItems} vendedores={data.vendedores}/>}
              {modulo==="egresos"        && <ModuloEgresos        egresos={data.egresos} onRegistrar={data.registrarEgreso} onReembolsar={data.marcarReembolsado} vendedores={data.vendedores}/>}
              ...
              {modulo==="abastecimiento" && <ModuloAbastecimiento productos={data.productos} abastecimiento={data.abastecimiento} onRegistrar={data.registrarAbastecimiento} vendedores={data.vendedores}/>}
              {modulo==="configuracion"  && <ModuloConfiguracion  vendedores={data.vendedores} onGuardar={data.guardarVendedor} onToggle={data.toggleVendedor}/>}
            </>)
          }
        </div>
      </div>
...
// ============================================================
function ModuloConfiguracion({vendedores,onGuardar,onToggle}){
  const [modal,  setModal]  = useState(false);
