...
- **Deuda por Traspasos en el card "Reembolsos pendientes"** (2026-08-10): solo en Caamaño (`localKey==="camanio"`) se agregó un tercer bloque que muestra cuánto le debe a Pilar por traspasos de mercadería sin saldar todavía — `traspasos.filter(t=>t.estado!=="pagado").reduce(...saldo_pendiente...)`. La tabla `traspasos` ya se replica en ambos locales y ya se cargaba siempre en `useData` (`cargarTraspasos()` no está gateada por local, solo `pagosTraspaso` lo está); lo único que faltaba era usarla en Egresos, porque el módulo `ModuloTraspasos` que sí la muestra en detalle es `soloPilar:true` en la navegación — Caamaño no tenía forma de ver ese saldo desde ningún lado.
