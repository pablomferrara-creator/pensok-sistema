...
- **No hay forma de reconstruir con certeza el historial exacto de stock** — `productos.stock` no tiene ninguna tabla de auditoría de cambios, así que no se pudo confirmar 100% si ESTE caso puntual fue afectado (al momento de revisar, el stock ya estaba en el valor correcto). Pero se encontró y confirmó un bug real en el mecanismo que hubiera podido causar justo este síntoma.
- **Bug confirmado**: `guardarVenta()` en `ModuloIngresos` (la función que corre al guardar "Editar venta") ajusta el stock comparando cantidades de items viejas vs. nuevas por producto, pero clampeaba el resultado con `Math.max(0, ...)` — a diferencia de `registrarVenta` (crear una venta nueva), que **no clampea y permite stock negativo a propósito** (es el mismo criterio de "vendido antes de abastecer" que ya usan `estadoStock` y la card "Pendiente de abastecer"). Si al editar una venta el producto ya estaba en o cerca de 0, el ajuste real se perdía en silencio, sin dejar ningún rastro de que faltó descontar. Sacado el `Math.max(0,...)` — ahora el ajuste por edición se comporta igual que al crear la venta.
- **Encontrado de paso, código muerto real**: `ModuloIngresos` tenía **dos funciones `guardarVenta` declaradas con el mismo nombre** (y una `agregarItemDesdeProducto` que solo acompañaba a la primera). En JS, una función redeclarada en el mismo scope queda pisada por la última declaración — así que la primera versión (más vieja, sin tocar stock para nada, ni con clamp ni sin él) nunca se ejecutaba desde ningún botón, pero quedaba ahí generando confusión real (casi lleva a pensar que había dos caminos de edición distintos con el mismo bug). Se eliminó por completo — no había ningún `onClick` que la llamara, confirmado antes de borrarla.
- **No se hizo una auditoría completa de stock actual vs. histórico** — sería poco confiable igual, porque los ajustes de Control de Stock (conteos aplicados) resetean el stock a lo contado sin dejar registro tampoco, así que una reconciliación simple de "entradas menos salidas" no da un número confiable desde el principio de los tiempos. Si en algún momento hace falta, lo más práctico es un conteo físico (Control de Stock) para la categoría en cuestión, no una reconstrucción histórica.
...
- Esto no era exclusivo de Productos: afecta a **todos** los módulos que guardan sus filtros como estado local (Egresos, Traspasos, Control de Stock, Abastecimiento, etc.). `ModuloIngresos` era la única excepción real porque en algún momento anterior sus filtros se "levantaron" al estado de `App` (`ingFiltros`/`setIngFiltros`, ver el comentario "Filtros persistentes por módulo (sobreviven recargas de datos)") — un parche puntual para este mismo problema, hecho antes de identificar la causa real.
- **Fix de raíz, no un parche por módulo**: nuevo estado `cargoInicial` en `App` — arranca en `false`, y un `useEffect` lo pasa a `true` la primera vez que `data.loading` se pone en `false` (primera carga de datos completa) y nunca vuelve a `false`. El spinner de pantalla completa ahora usa `!cargoInicial&&data.loading&&modulo!=="venta"` en vez de `data.loading&&modulo!=="venta"` — solo se muestra en el arranque de la app, nunca de nuevo. Los refrescos de datos en segundo plano (después de guardar algo, o el auto-refresh de 1h) siguen actualizando los props de cada módulo con normalidad, pero **ya no lo desmontan** — el spinner chico de la barra superior (`{data.loading&&<Spinner/>}`) sigue avisando que hay una recarga en curso.
- Con esto, los filtros/búsquedas de cualquier módulo sobreviven a guardar, editar o dar de baja algo. Se siguen limpiando solo con el botón "Limpiar filtros" de cada módulo, o con un refresh completo de la página (F5) — que sí vuelve a montar todo desde cero, como es esperable.
- `ingFiltros` (el parche puntual de Ingresos) se dejó intacto — ya no hace falta, pero no molesta.
