...
// CONSTANTES
// ============================================================
const VENDEDORES   = ["Fabri", "Maxi", "Pablo", "Kito"];
const METODOS_PAGO = ["Efectivo", "Transferencia", "Débito", "Crédito", "Mercado Pago", "Cuenta corriente"];
const MODALIDADES  = ["En el local", "Telefónica / Delivery"];
const CATEGORIAS   = ["Pileta", "Jardinería", "Limpieza", "Fumigación"];
const TIPOS_EGRESO = ["Gasto fijo", "Gasto variable", "Compra a proveedor", "Servicio", "Impuesto / Tasa", "Devolución"];
const PROVEEDORES  = ["Proveedor A", "Proveedor B", "Proveedor C", "Proveedor D"];
const PAGADORES    = ["Pensok", ...VENDEDORES];
const USD_RATE     = 1200;
...
function PantallaLogin({onLogin}){
  ...
  async function handleSubmit(){
    ...
    if(modo==="login"){
      ...
      else{setError("");setModo("login");setError("Cuenta creada. Chequeá tu email para confirmar.");}
    }
    setLoading(false);
  }
    ...
    <div style={{minHeight:"100vh",background:G.fondo,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <div style={{fontSize:13,color:G.textoSec,marginTop:4}}>Sistema de gestión</div>
        </div>
        <Card>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Fi label="Email" value={email} onChange={setEmail} type="email" placeholder="tu@email.com"/>
            <Fi label="Contraseña" value={pass} onChange={setPass} type="password" placeholder="••••••••"/>
            ...
            <div style={{textAlign:"center",fontSize:12,color:G.textoSec}}>
              {modo==="login"?"¿Primera vez? ":"¿Ya tenés cuenta? "}
...
function useData(toast){
  ...
  async function cargar(){
    setLoading(true);
    const[{data:cls},{data:prds},{data:vts},{data:vis},{data:egs},{data:abs}]=await Promise.all([
    ...
    setClientes(cls||[]);setProductos(prds||[]);setVentas(vts||[]);setVentaItems(vis||[]);setEgresos(egs||[]);setAbastecimiento(abs||[]);
    setLoading(false);
  }
  ...
  const ventasConItems = useMemo(()=>
    ventas.map(v=>({...v,items:(ventaItems.filter(i=>i.venta_id===v.id)||[])}))
  ,[ventas,ventaItems]);
  ...
  async function registrarAbastecimiento(datos){
    ...
    toast.ok("Ingreso de mercadería registrado");
    await cargar();
  }

  return{clientes,productos,ventasConItems,egresos,abastecimiento,loading,cargar,registrarVenta,registrarEgreso,marcarReembolsado,guardarCliente,guardarProducto,registrarAbastecimiento};
}

// ============================================================
// MÓDULO: ANÁLISIS / DASHBOARD
// ============================================================
function ModuloAnalisis({ventas,egresos,productos}){
  ...
  const porVend=VENDEDORES.map(v=>({v,total:vMes.filter(x=>x.vendedor===v).reduce((s,x)=>s+(x.total||0),0),cant:vMes.filter(x=>x.vendedor===v).length})).sort((a,b)=>b.total-a.total);
...
// MÓDULO: NUEVA VENTA
// ============================================================
function ModuloVenta({clientes,productos,onRegistrar}){
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <Card>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Fi label="Vendedor"       value={vendedor}  onChange={setVendedor}  options={VENDEDORES}/>
            <Fi label="Método de pago" value={metodo}    onChange={setMetodo}    options={METODOS_PAGO}/>
        ...
        <Card>
          ...
          <div style={{position:"relative"}}>
            <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Buscar por nombre, código o categoría..."
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
        <MetricCard label="Ventas"        value={fmtNum(filtrados.length)}/>
      ...
      <Card style={{padding:"12px 18px"}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
          <Fi value={fVend} onChange={setFV} options={["Todos",...VENDEDORES]}   style={{flex:1,minWidth:130}}/>
          ...
          {fFecha&&<Btn small variant="ghost" onClick={()=>setFF("")}>Limpiar</Btn>}
        </div>
      </Card>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {filtrados.map(v=>(
          <Card key={v.id} style={{padding:"12px 18px"}}>
            ...
            {v.items?.length>0&&<div style={{marginTop:8,fontSize:12,color:G.textoSec}}>{v.items.map(i=>`${i.nombre} ×${i.cantidad}`).join(" · ")}</div>}
          </Card>
        ))}
        {filtrados.length===0&&<div style={{textAlign:"center",padding:"48px 0",color:G.textoSec}}>Sin ventas en esta selección</div>}
      </div>
    </div>
  );
}
...
// MÓDULO: EGRESOS
// ============================================================
function ModuloEgresos({egresos,onRegistrar,onReembolsar}){
  ...
  const deudasPers=VENDEDORES.map(v=>({persona:v,deuda:egresos.filter(e=>e.pagador===v&&e.reembolso_pendiente&&!e.reembolsado).reduce((s,e)=>s+(e.monto||0),0)})).filter(d=>d.deuda>0);
  const colorT={"Gasto fijo":"azul","Gasto variable":"gris","Compra a proveedor":"verde","Servicio":"violeta","Impuesto / Tasa":"naranja","Devolución":"rojo"};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <MetricCard label="Selección"        value={fmt(totalF)}/>
      ...
      <Card style={{padding:"12px 18px"}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
          ...
          <Fi value={filtroP} onChange={setFP} options={["Todos",...PAGADORES]}     style={{flex:1,minWidth:130}}/>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
              <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:12,color:G.textoSec,marginTop:2}}>{e.fecha} · {e.metodo_pago} · Pagó: {e.pagador}{e.proveedor&&` · ${e.proveedor}`}</div>
                  <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
        ...
        <Modal title="Registrar egreso" onClose={()=>setModal(false)}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <Fi label="Método pago" value={fMetodo}  onChange={setFMet}   options={METODOS_PAGO}/>
              <Fi label="¿Quién pagó?" value={fPagador} onChange={setFPag}  options={PAGADORES}/>
              <Fi label="Proveedor"   value={fProv}    onChange={setFProv}  options={["", ...PROVEEDORES]}/>
            </div>
    ...
    <div style={{display:"flex",gap:16,alignItems:"start"}}>
                  ...
                  <div style={{display:"flex",gap:12,alignItems:"center"}}>
                    ...
                    <div>
                      ...
                      <div style={{display:"flex",gap:6,marginTop:5}}>
                        ...
                        {(c.limite_cuenta||0)>0&&<Badge color="gris">Límite {fmt(c.limite_cuenta)}</Badge>}
                      </div>
                    </div>
                  </div>
        ...
        <div style={{width:290,flexShrink:0,position:"sticky",top:60}}>
          <Card>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,fontSize:13}}>
              {clienteSelec.telefono&&<div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:G.textoSec}}>Teléfono</span><span>{clienteSelec.telefono}</span></div>}
              ...
              {clienteSelec.direccion&&<div style={{display:"flex",justifyContent:"space-between",gap:8}}><span style={{color:G.textoSec,flexShrink:0}}>Dirección</span><span style={{textAlign:"right",fontSize:12}}>{clienteSelec.direccion}</span></div>}
            </div>
            <Div/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              ...
              <div style={{background:G.sup2,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:10,color:G.textoSec,textTransform:"uppercase",letterSpacing:0.5}}>Límite</div><div style={{fontSize:16,fontWeight:700,color:G.textoSec,fontFamily:"'DM Mono',monospace"}}>{(clienteSelec.limite_cuenta||0)>0?fmt(clienteSelec.limite_cuenta):"—"}</div></div>
            </div>
            <Div/>
            <ST>Historial</ST>
      ...
      {modal&&(
        <Modal title={editando?"Editar cliente":"Nuevo cliente"} onClose={()=>setModal(false)}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Fi label="Nombre / razón social" value={fNombre} onChange={setFN} placeholder="Ej: Club Náutico Pilar"/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Tipo de precio"  value={fTipo}  onChange={setFTipo}  options={["minorista","especial","mayorista"]}/>
              <Fi label="Teléfono"        value={fTel}   onChange={setFTel}   placeholder="0230-444-0000"/>
              <Fi label="Email"           value={fEmail} onChange={setFEmail} type="email" placeholder="contacto@empresa.com"/>
              <Fi label="Límite cta. cte." value={fLimite} onChange={setFLim} type="number" placeholder="0"/>
            </div>
            <Fi label="Dirección" value={fDir}   onChange={setFDir}   placeholder="Calle y número, localidad"/>
            <Fi label="Notas"     value={fNotas} onChange={setFNotas} rows={2} placeholder="Ej: pago a 30 días..."/>
          </div>
        </Modal>
      )}
    </div>
...
// MÓDULO: PRODUCTOS
// ============================================================
function ModuloProductos({productos,onGuardar}){
  const [busqueda,setB]=useState(""); const [filtroC,setFC]=useState("Todas"); const [filtroE,setFE]=useState("Todos");
  ...
  const [fMoneda,setFMon]=useState("ARS"); const [fCosto,setFCosto]=useState(""); const [fMin,setFMin]=useState("");
  const [fEsp,setFEsp]=useState(""); const [fMay,setFMay]=useState(""); const [fStock,setFStock]=useState("");
  const [fStockMin,setFStockMin]=useState(""); const [fProv,setFProv]=useState(PROVEEDORES[0]);
  const [loading,setLoading]=useState(false);

  function abrirEditar(p){setEditando(p);setFK(p.codigo);setFN(p.nombre);setFCat(p.categoria);setFMon(p.moneda);setFCosto(String(p.costo));setFMin(String(p.precio_min));setFEsp(String(p.precio_esp||0));setFMay(String(p.precio_may||0));setFStock(String(p.stock));setFStockMin(String(p.stock_min));setFProv(p.proveedor||PROVEEDORES[0]);setModal(true);}
  function abrirNuevo(){setEditando(null);setFK("");setFN("");setFCat(CATEGORIAS[0]);setFMon("ARS");setFCosto("");setFMin("");setFEsp("");setFMay("");setFStock("");setFStockMin("");setFProv(PROVEEDORES[0]);setModal(true);}

  async function guardar(){
    if(!fCodigo||!fNombre||!fCosto||!fMin)return;setLoading(true);
    const datos={codigo:fCodigo,nombre:fNombre,categoria:fCat,moneda:fMoneda,costo:parseFloat(fCosto),precio_min:parseFloat(fMin),precio_esp:parseFloat(fEsp)||0,precio_may:parseFloat(fMay)||0,stock:parseInt(fStock)||0,stock_min:parseInt(fStockMin)||0,proveedor:fProv,activo:true};
    await onGuardar(datos,editando?.id||null);
    setLoading(false);setModal(false);
  }

  const alertas=productos.filter(p=>p.activo&&estadoStock(p)!=="ok");
  const valorStock=productos.reduce((s,p)=>s+precioARS(p.costo,p.moneda)*p.stock,0);
  const filtrados=useMemo(()=>productos.filter(p=>{if(!p.activo&&filtroE!=="Inactivos")return false;if(filtroC!=="Todas"&&p.categoria!==filtroC)return false;if(filtroE==="Bajo stock"&&estadoStock(p)!=="bajo")return false;if(filtroE==="Agotados"&&estadoStock(p)!=="agotado")return false;if(filtroE==="Inactivos"&&p.activo)return false;if(busqueda){const q=busqueda.toLowerCase();if(!p.nombre.toLowerCase().includes(q)&&!p.codigo.toLowerCase().includes(q))return false;}return true;}),[productos,busqueda,filtroC,filtroE]);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {alertas.length>0&&(
        <Card style={{background:"#FF4D6A06",border:`1px solid #FF4D6A22`,padding:"12px 18px"}}>
          <div style={{fontSize:12,fontWeight:600,color:G.rojo,marginBottom:8}}>⚠ Reposición necesaria</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {alertas.map(p=>(
              <div key={p.id} style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:8,padding:"6px 12px",fontSize:12}}>
                <span style={{fontWeight:500}}>{p.nombre}</span>
                <span style={{color:estadoStock(p)==="agotado"?G.rojo:G.amarillo,marginLeft:8,fontFamily:"'DM Mono',monospace"}}>{p.stock===0?"AGOTADO":`${p.stock}/${p.stock_min}`}</span>
                <span style={{color:G.textoSec,marginLeft:6}}>→ pedir {Math.max(0,p.stock_min*2-p.stock)}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <Card style={{padding:"12px 18px"}}>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:180}}><input value={busqueda} onChange={e=>setB(e.target.value)} placeholder="Buscar..." style={{background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:8,padding:"8px 12px",color:G.texto,fontSize:13,width:"100%",outline:"none"}}/></div>
          <Fi value={filtroC} onChange={setFC} options={["Todas",...CATEGORIAS]} style={{width:130}}/>
          <Fi value={filtroE} onChange={setFE} options={["Todos","Bajo stock","Agotados","Inactivos"]} style={{width:130}}/>
      ...
      <div style={{background:G.sup,border:`1px solid ${G.borde}`,borderRadius:12,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{borderBottom:`1px solid ${G.borde}`}}>{["Código","Producto","Cat.","Stock","Costo","Minorista","Especial","Mayorista","Vendidos","Estado",""].map(h=><th key={h} style={{padding:"10px 12px",textAlign:"left",color:G.textoSec,fontWeight:500,fontSize:11,textTransform:"uppercase",letterSpacing:0.5,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
            <tbody>
              {filtrados.map((p,i)=>{
                ...
                return(
                  <tr key={p.id} style={{borderBottom:`1px solid ${G.borde}22`,background:i%2===0?"transparent":G.sup2+"44"}}>
                    <td style={{padding:"9px 12px",fontFamily:"'DM Mono',monospace",fontSize:11,color:G.textoSec}}>{p.codigo}</td>
                    <td style={{padding:"9px 12px",fontWeight:500}}>{p.nombre}{p.moneda==="USD"&&<Badge color="usd" small> USD</Badge>}</td>
                    <td style={{padding:"9px 12px"}}><Badge color="gris">{p.categoria}</Badge></td>
                    <td style={{padding:"9px 12px",fontFamily:"'DM Mono',monospace",fontWeight:600,color:e==="agotado"?G.rojo:e==="bajo"?G.amarillo:G.texto}}>{p.stock}<span style={{color:G.textoSec,fontWeight:400,fontSize:11}}> /{p.stock_min}</span></td>
                    <td style={{padding:"9px 12px",fontFamily:"'DM Mono',monospace",color:G.textoSec,whiteSpace:"nowrap"}}>{pF(p.costo)}</td>
                    <td style={{padding:"9px 12px",fontFamily:"'DM Mono',monospace",whiteSpace:"nowrap"}}>{pF(p.precio_min)}</td>
                    ...
                    <td style={{padding:"9px 12px",fontFamily:"'DM Mono',monospace",color:G.verde}}>{fmtNum(p.vendidos)}</td>
                    <td style={{padding:"9px 12px"}}><Badge color={colorE[e]}>{labelE[e]}</Badge></td>
      ...
      {modal&&(
        <Modal title={editando?"Editar producto":"Nuevo producto"} onClose={()=>setModal(false)} maxWidth={540}
          footer={<><Btn variant="secondary" onClick={()=>setModal(false)}>Cancelar</Btn><Btn disabled={!fCodigo||!fNombre||!fCosto||!fMin||loading} onClick={guardar}>{loading?<span style={{display:"flex",alignItems:"center",gap:6}}><Spinner/>Guardando</span>:"Guardar producto"}</Btn></>}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Código"    value={fCodigo} onChange={setFK}    placeholder="PIL001"/>
              <Fi label="Categoría" value={fCat}    onChange={setFCat}  options={CATEGORIAS}/>
            </div>
            <Fi label="Nombre" value={fNombre} onChange={setFN} placeholder="Ej: Cloro líquido 5L"/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Proveedor" value={fProv}  onChange={setFProv} options={PROVEEDORES}/>
              <Fi label="Moneda"    value={fMoneda} onChange={setFMon} options={["ARS","USD"]}/>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
              <Fi label="Costo"     value={fCosto} onChange={setFCosto} type="number" placeholder="0"/>
              <Fi label="Minorista" value={fMin}   onChange={setFMin}   type="number" placeholder="0"/>
              <Fi label="Especial"  value={fEsp}   onChange={setFEsp}   type="number" placeholder="0"/>
              <Fi label="Mayorista" value={fMay}   onChange={setFMay}   type="number" placeholder="0"/>
            </div>
            {fCosto&&fMin&&<div style={{background:G.sup2,borderRadius:8,padding:"10px 14px",fontSize:12,color:G.textoSec,display:"flex",gap:20}}><span>Margen min: <strong style={{color:G.verde}}>{Math.round((fMin-fCosto)/fCosto*100)}%</strong></span>{fMay&&<span>Margen may: <strong style={{color:G.azul}}>{Math.round((fMay-fCosto)/fCosto*100)}%</strong></span>}</div>}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Fi label="Stock actual" value={fStock}    onChange={setFStock}    type="number" min="0" placeholder="0"/>
              <Fi label="Stock mínimo" value={fStockMin} onChange={setFStockMin} type="number" min="0" placeholder="0"/>
            </div>
          </div>
        </Modal>
      )}
...
// ============================================================
function ModuloAbastecimiento({productos,abastecimiento,onRegistrar}){
  ...
  const [proveedor,setProv]=useState(PROVEEDORES[0]); const [metodo,setMet]=useState(METODOS_PAGO[0]);
  const [resp,setResp]=useState(VENDEDORES[0]); const [notas,setNotas]=useState("");
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
        ...
        <MetricCard label="Último ingreso"   value={abastecimiento[0]?.fecha||"—"} sub={abastecimiento[0]?.nombre||""}/>
      </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:16,alignItems:"start"}}>
          <Card>
            ...
            <div style={{position:"relative"}}>
              ...
              {!prodSelec&&prodFilt.length>0&&(
                <div style={{position:"absolute",top:"100%",left:0,right:0,background:G.sup2,border:`1px solid ${G.borde}`,borderRadius:8,marginTop:4,zIndex:10,maxHeight:200,overflowY:"auto"}}>
                  {prodFilt.map(p=>(
                    <div key={p.id} onClick={()=>{setPS(p);setPB("");setCU(String(p.costo));setProv(p.proveedor||PROVEEDORES[0]);}} style={{padding:"9px 14px",cursor:"pointer",borderBottom:`1px solid ${G.borde}22`}} onMouseEnter={e=>e.currentTarget.style.background=G.borde} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                      <div style={{fontSize:13,fontWeight:500}}>{p.nombre}</div>
                      <div style={{fontSize:11,color:G.textoSec}}>Stock: {p.stock} · Último costo: {p.moneda==="USD"?fmtUSD(p.costo):fmt(p.costo)}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <Fi label="Proveedor"  value={proveedor} onChange={setProv} options={PROVEEDORES}/>
              <Fi label="Método pago" value={metodo}   onChange={setMet} options={METODOS_PAGO}/>
              <Fi label="Responsable" value={resp}     onChange={setResp} options={VENDEDORES}/>
            </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {abastecimiento.map(a=>(
            <Card key={a.id} style={{padding:"12px 18px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
                <div><div style={{fontWeight:600,fontSize:14}}>{a.nombre}</div><div style={{fontSize:12,color:G.textoSec,marginTop:2}}>{a.fecha} · {a.proveedor} · {a.metodo_pago} · {a.responsable}</div>{a.notas&&<div style={{fontSize:11,color:G.textoSec,marginTop:2,fontStyle:"italic"}}>{a.notas}</div>}</div>
                <div style={{textAlign:"right"}}><div style={{fontSize:17,fontWeight:700,color:G.naranja,fontFamily:"'DM Mono',monospace"}}>{fmt((a.cantidad||0)*(a.costo_unit||0))}</div><div style={{fontSize:11,color:G.textoSec}}>{fmtNum(a.cantidad)} u. × {fmt(a.costo_unit)}</div></div>
              </div>
            </Card>
          ))}
...
export default function App(){
  ...
  if(checking) return(
    <div style={{minHeight:"100vh",background:G.fondo,display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
      <Spinner/><span style={{color:G.textoSec}}>Verificando sesión...</span>
    </div>
  );
      ...
      <div style={{minHeight:"100vh",background:G.fondo}}>
        <div style={{background:G.sup,borderBottom:`1px solid ${G.borde}`,padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:50,position:"sticky",top:0,zIndex:50}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <span style={{color:G.textoSec,fontSize:12}}>gestión</span>
          </div>
        ...
        <div style={{padding:"20px 22px",maxWidth:1200,margin:"0 auto"}}>
          {data.loading&&modulo!=="venta"
            ...
            :(<>
              {modulo==="analisis"       && <ModuloAnalisis       ventas={data.ventasConItems} egresos={data.egresos} productos={data.productos}/>}
              {modulo==="venta"          && <ModuloVenta          clientes={data.clientes} productos={data.productos} onRegistrar={data.registrarVenta}/>}
              {modulo==="ingresos"       && <ModuloIngresos       ventas={data.ventasConItems}/>}
              {modulo==="egresos"        && <ModuloEgresos        egresos={data.egresos} onRegistrar={data.registrarEgreso} onReembolsar={data.marcarReembolsado}/>}
              {modulo==="clientes"       && <ModuloClientes       clientes={data.clientes} onGuardar={data.guardarCliente} ventas={data.ventasConItems}/>}
              {modulo==="productos"      && <ModuloProductos      productos={data.productos} onGuardar={data.guardarProducto}/>}
              {modulo==="abastecimiento" && <ModuloAbastecimiento productos={data.productos} abastecimiento={data.abastecimiento} onRegistrar={data.registrarAbastecimiento}/>}
            </>)
          }
        </div>
      </div>
